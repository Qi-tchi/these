...
We first extended an existing technique, the type graph method, to non-well-founded semirings, improving its usability.
Then, a new automated termination technique, called morphism counting, was proposed for injective DPO graph rewriting systems. It is based on counting monomorphisms from specific pattern-graphs to the graph before and after transformation. This technique can handle cases, such as Example~\ref{subgraph_counting:ex_contrib_variant}, that existing interpretation-based approaches~\cite{zantema2014termination,bruggink2014termination,bruggink2015proving,endrullis2024generalized_icgt,overbeek2024termination_lmcs} cannot.
...
in Examples~\ref{antipattern:ex:grs_aca} and~\ref{antipattern:ex:endrullis:d3:termination} which the morphism counting method presented in Chapter~\ref{chap:subgraph_counting} fails to handle.
...
In the long term (5+ years), we aim to:
    \begin{itemize}
      \item Extend the type graph method and the morphism counting technique to other algebraic graph rewriting formalisms such as PBPO+~\cite{overbeek2023graph}.
