...
For example, consider the graph $\mathcal{G}$ shown in the following~\autoref{fig:graph_modeling_state_network}, enclosed in a box labeled with its name in the top-left corner. 
...
The algorithm operates as follows: when an active unit detects a neutral neighbor via a channel in state $0$, it activates the neighbor and updates the channel to state $1$.
 This behavior is captured by the graph transformation rule in the following~\autoref{fig:intro:graph_transformation_rule_0}.
 \begin{figure}[H]
        \centering 
...
\resizebox{0.3\textwidth}{!}{
        ...
        \caption{Sequence of graph transformation, to be read from left to right and from top to bottom.}
...
This enables a uniform definition of graph transformation, and leads to an elegant definition of graph transformation. 
 ...
 As an example, the graph transformation rule, previously defined and illustrated in~\autoref{fig:intro:graph_transformation_rule_0}, can be represented as the DPO graph rewriting rule shown in~\autoref{fig:intro:graph_transformation_rule_0_dpo}.
...
\newpage 
As an example, 
consider the graph transformation rule, shown in the following~\autoref{fig:intro:graph_transformation_rule_dpo_sssssfggaaadww}, and the graphs shown in the following~\autoref{fig:intro:dangling_edge}.
...
\begin{figure}[H]
    ...
    \caption{}
\end{figure}
\noindent The graph $\mathcal{G}$ shown in~\autoref{fig:intro:graph_G} can be rewritten to yield the graph $\mathcal{H}$ shown in~\autoref{fig:intro:graph_H} by applying the following DPO graph rewriting rule, defined earlier,
% in~\autoref{fig:intro:graph_transformation_rule_dpo} 
...
For example, consider the rule, defined earlier and shown in~\autoref{fig:intro:graph_transformation_rule_dpo_sssssfggaaadww}. and the graph $\mathcal{G}'$ in~\autoref{fig:intro:dangling_edge}. The graph $\mathcal{G}'$ is obtained from the graph $\mathcal{G}$ in~\autoref{fig:intro:graph_G} by adding an edge from $3$ to $6$ (in red). This graph has the same occurrence of the graph $\mathcal{L}$ as the graph $\mathcal{G}$ in~\autoref{fig:intro:graph_G}.
...
\raisebox{2pt}{
    ...
    A looping sequence of graphs, each obtained by applying the rule to its predecessor, is shown in the following~\autoref{fig:intro:sequence_of_transformation_infinite}. In each graph,
    the subgraph replaced at each transformation step is highlighted in red.
         ...
         \end{tikzpicture}
         \caption{Sequence of rewriting steps, to be read left to right.}
...
For example, consider the graph transformation system with two rules shown below.

% \begin{center}
% in the following~\autoref{fig:intro:edge_deletion_and_node_addition_rule}
  \begin{figure}[H]
        \centering
...
%         % \centering
    ...
    % \end{subfigure} 
    \caption{A rewriting system with two rules.}
    \label{fig:intro:edge_deletion_and_node_addition_rule}
  \end{figure}
% \end{center}

\noindent Rule $\alpha$ deletes an arbitrary edge labeled by $A$, and rule $\beta$ introduces a fresh node. The system does not terminate because the node-adding rule $\beta$ can be applied indefinitely. However, the edge-deleting rule $\alpha$ can be applied a finite number of times only: it deletes an edge on each application, and since no rule increases the edge count and the initial graph is finite, only finitely many deletions are possible. Therefore, termination of the full system depends solely on the node-adding rule $\beta$. This observation motivates the notion of \emph{relative termination}.
...
As an example, consider the edge-deleting rule $\alpha$ and the node-adding rule $\beta$, defined earlier and shown in~\autoref{fig:intro:edge_deletion_and_node_addition_rule}. 
...
%  shown in~\autoref{fig:intro:graph_transformation_rule_anti_pattern__}.
