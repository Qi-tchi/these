...
We present a method for termination analysis of DPO graph rewriting systems with injective rules.\todo{Section 4 is quite perplexing. The paper emphasizes that the decision procedure is designed for injective matches, but the reader only learns here that it also works for epis. It would be beneficial to justify why injective matches make a difference (I suspect this is because they can be handled algorithmically).}
...
\begin{definition}
    ...
    \).
\end{definition} 
\trackedtext{The occurrence count of ruler-graphs before and after a rewriting step provides a termination criterion: 
    ...
    \trackedtext{For some ruler-graphs, occurrence changes during a rewriting step using an injective rule are precisely computable by simply counting their occurrences in the left- and right-hand side graphs. Examples include: the ruler-graph \tikz[baseline=-0.5ex]{
        \node (x) at (0,0) {$\bullet$};
    ...
    \trackedtext{However, systems like \autoref{ex:grsaa_rx} require more complex ruler-graphs such as \tikz[baseline=-0.5ex]{
        \node (x) at (0,0) {$\bullet$};
        \node (y) at (1,0) {$\bullet$ };
        \node (z) at (2,0) { $\bullet$};
    ...
    \trackedtext{To combine measurements from multiple ruler-graphs (analogous to combining measurements of physical objects from multiple rulers), we introduce:}
...
% \subsection{Measuring graphs with ruler-Graphs}
% \label{sec:interpretation}
% \trackedtext{
...
% % \end{remark}
% % \begin{remark}
%     \trackedtext{Trivial ruler-graphs include graphs with a single node and no edges, graphs with a single node and a single labeled loop, and graphs with two nodes and a single labeled edge between them. 
...
\begin{lemma}
    \label{lem:xlnlmxrnr}
    \todo{j'ai remplace \text{$L \overset{l}{\leftarrow} K \overset{r}{\rightarrow} R$} par \text{$L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R$}}
    Let $X$ be a graph. \trackedtext{Let $L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R$} be an injective DPO graph rewriting rule. We have 
    \[
       \card{\operatorname{Mono}(X, L, \lnot l)}  - \card{\operatorname{Mono}(X, R, \lnot r)} 
       = 
       \card{\operatorname{Mono}(X, L)}  - \card{\operatorname{Mono}(X, R)} 
...
\begin{minipage}{0.59\textwidth}
    % \setlength{\parindent}{1em}
    \trackedtext{Let \( \rho = (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R) \) be an injective DPO rewriting rule. 
...
\end{minipage}
 \trackedtext{the lower right diagram, $L', C', R', K$ are mutually disjoint sets such that $K$, 
         $K \uplus L'$, 
         $K \uplus R'$, 
         $K \uplus C'$, 
...
\trackedtext{With the above DPO diagrams in mind, we define:}
...
\begin{definition}
    \label{def:corresponding_occurrence}
     ...
     \trackedtext{The occurrence $x$ can be decomposed as the union of graphs $x = R_x \cup C_x$ where $R_x = x \cap R$ and $C_x = x \cap C$. For every graph monomorphism $h : R_x \rightarrowtail L$ that preserves interface elements (i.e., \( h(k) = k \) for all \( k \in K \)), we define the \textbf{corresponding $X$-occurrence of $x$ relative to $h$} as the $X$-occurrence in $G$ given by $h(R_x) \cup C_x$.}
...
\begin{example}
    ...
    \end{center}
    Let $R' \in D(R,X)$. \trackedtext{$R'$ must contain node 3, otherwise it would be a subgraph of $r(K)$, violating Condition 2 of \autoref{def:rx}. Node 3 must be connected to an interface node, otherwise Condition 1 of \autoref{def:rx} would be violated. Therefore, $R'$ must include \raisebox{2pt}{\scalebox{0.6}{\tikz[baseline=-0.5ex]{
...
\end{example}
We define the notion of \( X \)-non-increasing rules. Intuitively, \trackedtext{if} a rule $\rho$ is $X$-non-increasing, \trackedtext{then} for any rewriting step using $\rho$, there exists an injective mapping from the set of \( X \)-occurrences implicitly created by the step to the set of \( X \)-occurrences implicitly destroyed by the step. This will be proved in \autoref{lem:w_u_l_not_geq_r_not}.
\begin{definition}[Non-increasing rule]
    ...
    \begin{enumerate}
        \item For all $R' \in D(R,X)$, there exists a morphism \(h_{R'L}: R' \rightarrowtail L \) such that \trackedtext{both squares in the diagram shown on the right are pullbacks}, where all morphisms other than $h_{R'L}$, $l$ and $r$ are inclusion functions, 
        \item \label{def:non_increasing:non_clapse} \trackedtext{For all $R' \in D(R,X)$,} for all node or edge $r'$ in $R'$, if $ r' \notin \operatorname{Im}(r)$ then $h_{R'L}(r') \notin \operatorname{Im}(l)$,
        ...
        \begin{center}
            % \resizebox{0.9\textwidth}{!}{ 
                ...
                \begin{tikzpicture} 
                    ...
                    \draw[>->]  (k') -- (rb);
                    \draw[>->]  (k') -- (k);
                    \draw[>->]  (rb) -- (l) node [midway,above] {\trackedtext{$h_{R'L}$}};
                    \node () [at=($(k)!0.5!(rb)$)] {PB};
                    \node () [at=($(k)!0.5!(rb')$)] {PB};
                \end{tikzpicture}
            % }
...
\end{definition}
\trackedtext{The first condition ensures that whenever $R' \in D(R,X)$ forms an $X$-occurrence in the result graph $H$, this occurrence has a corresponding $X$-occurrence relative to the morphism $h_{R'L}$ (as defined in \autoref{def:corresponding_occurrence}) in the host graph $G$.}
\trackedtext{The second condition guarantees that any $X$-occurrence implicitly created by the rewriting step has its corresponding $X$-occurrence not included in the context (Consequently, its corresponding $X$-occurrence is implicitly destroyed by the rewriting step). Counterexamples are provided in \autoref{ex:cond_2_necessaire} and \autoref{ex:cond_2_necessaire2}.}
\trackedtext{The third and fourth conditions ensure that distinct $X$-occurrences \trackedtext{created implicitly} have distinct corresponding $X$-occurrences.
Counterexamples for these conditions are given in \autoref{ex:cond3_necessaire} and \autoref{ex:cond4_necessaire}.}

\trackedtext{The following example shows an $X$-non-increasing rule.}
\begin{example} 
    ...
    in \autoref{ex:grsaa_rx}. By \autoref{ex:grsaa_rx}, the set \( D(R,X) \) \trackedtext{consists of exactly four graphs: $R'_1$, $R'_2$, $R'_3$ and $R'_4$}.
    The rule is $X$-non-increasing because the following diagrams can be constructed, and \trackedtext{$h_{R'_1L}, h_{R'_2L}, h_{R'_3L}$ and $h_{R'_4L}$} satisfy all conditions in \autoref{def:creates_more_x_on_the_left}. 
\trackedtext{%1
\begin{center}
    \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
            ...
            \node () at (11.5mm,-17.5mm) {\trackedtext{\trackedtext{$h_{R'_1L}$}}};
...
\begin{center}
    \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
            ...
            \node () at (37mm,-8mm) {$\leftarrowtail$};
            \node () at (17mm,-17mm) {$\downarrowtail$};
            \node () at (11.5mm,-17.5mm) {\trackedtext{$h_{R'_2L}$}};
            \node () at (52mm,-17mm) {$\downarrowtail$};
            \node () at (92mm,-17mm) {$\downarrowtail$};
            \node () at (37mm,-18mm) {\text{PB}};
            \node () at (68mm,-18mm) {\text{PB}};
...
\begin{center}
    \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
            ...
            \node () at (37mm,-8mm) {$\leftarrowtail$};
            \node () at (17mm,-17mm) {$\downarrowtail$};
            \node () at (11.5mm,-17.5mm) {\trackedtext{$h_{R'_3L}$}};
            \node () at (52mm,-17mm) {$\downarrowtail$};
            \node () at (92mm,-17mm) {$\downarrowtail$};
            \node () at (37mm,-18mm) {\text{PB}};
            \node () at (68mm,-18mm) {\text{PB}};
...
\begin{center}
    \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
            ...
            \node () at (37mm,-8mm) {$\leftarrowtail$};
            \node () at (17mm,-17mm) {$\downarrowtail$};
            \node () at (11.5mm,-17.5mm) {\trackedtext{$h_{R'_4L}$}};
            \node () at (52mm,-17mm) {$\downarrowtail$};
            \node () at (92mm,-17mm) {$\downarrowtail$}; 
            \node () at (37mm,-18mm) {\text{PB}};
            \node () at (68mm,-18mm) {\text{PB}};
...
To illustrate the $X$-non-increasing property, consider the rewriting step defined by the DPO diagram below. \trackedtext{Whenever \( R'_i \) (for \( 1 \leq i \leq 4 \)) forms an \( X \)-occurrence $x$ that is included neither in \( R \) nor $C$ with a subgraph $C' \subseteq C \cap x \subseteq C$, the graph \( h_{R'_iL}(R'_i) \) forms a \( X \)-occurrence in $G$ with $C'$.} Furthermore, distinct $X$-occurrences created in $H$ have distinct corresponding $X$-occurrences in $G$.
    \begin{center} 
        \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
...
\begin{example} 
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            \draw[->] (y) -- (z)   {};
    }. The following rewriting rule \trackedtext{is potentially $X$-increasing}.
    \begin{center}
        \resizebox{0.7\textwidth}{!}{
            \begin{tikzpicture}
                \graphbox{$L$}{0mm}{0mm}{34mm}{15mm}{2mm}{-5mm}{
    ...
    \end{center}
    As shown in \autoref{ex:rx_counter_ex}, \( D(R,X) \) contains \trackedtext{the graph} $R'$:
    \raisebox{2pt}{
...
\begin{example}
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \node (z) at (2,0) {$\bullet$};
            \draw[->] (x) -- (y)   {};
    }, which has an isolated node. The following rewriting rule \trackedtext{is potentially $X$-increasing}.
    \begin{center}
        \resizebox{0.6\textwidth}{!}{
            \begin{tikzpicture}
                \graphbox{$L$}{0mm}{0mm}{30mm}{20mm}{0}{0}{
