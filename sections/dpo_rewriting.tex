...
A \textbf{DPO rewriting rule}\index{DPO rewriting rule} $\rho$ is a span \( L \overset{l}{\leftarrow} K \overset{r}{\rightarrow} R \), where \( K \) is the \textbf{interface}\index{Interface}, \( L \) is the \textbf{left-hand-side object}\index{Left-hand-side object}, denoted by \( \operatorname{lhs}(\rho) \), and \( R \) is the \textbf{right-hand-side object}\index{Right-hand-side object}, denoted by \( \operatorname{rhs}(\rho) \). The rule $(R \overset{r}{\leftarrow} K \overset{l}{\rightarrow} L)$ is denoted by $\rho^{-1}$. 

The rule is said to be \textbf{left monic}\index{Left monic rule} if \( l \) is monic, and \textbf{right monic}\index{Right monic rule} if \( r \) is monic; 
it is said to be \textbf{monic}\index{Monic rule} if it is both left monic and right monic.
...
The diagram with double pushout in Definition~\ref{def:rewriting_step} is said to be a \textbf{witness}\index{Witness!of a rewriting step} for the \textbf{rewriting step}\index{Rewriting step} from \( G \) to \( H \) using
...
% \end{figure}
    The \textbf{DPO rewriting relation $\mathop{\Rightarrow}_{\rho,\mathfrak{F}}$ induced by a DPO rewriting rule $\rho$ in $\mathfrak{F}$}\index{DPO rewriting relation}\index{$\mathop{\Rightarrow}_{\rho,\mathfrak{F}}$} is defined as follows:
    ...
    The \textbf{DPO rewriting relation $\mathop{\Rightarrow}_{\mathcal{R},\mathfrak{F}}$ induced by a set $\mathcal{R}$ of DPO rewriting rules in $\mathfrak{F}$}\index{$\mathop{\Rightarrow}_{\mathcal{R},\mathfrak{F}}$} is given by: 
