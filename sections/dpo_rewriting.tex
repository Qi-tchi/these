...
\begin{example}
  \label{ex:grsaa}
  Consider the DPO rewriting rule shown in Figure~\ref{fig:preliminaries:a_rewriting_rule} from \cite[Example 6]{bruggink2014termination}. 
...
As an example, consider the DPO rewriting rule shown in Figure~\ref{fig:preliminaries:a_rewriting_rule_sfdsgdfa} and the DPO diagram in Figure~\ref{fig:preliminaries:a_rewriting_step}.
\begin{figure}[H]
      \centering 
...
However, given a match $L \overset{m}{\rightarrow} G$, the left pushout square cannot always be constructed. For example, consider the same rewriting rule, shown below.
%  in Figure~\ref{fig:preliminaries:a_rewrdfagiting_rule_sfdsgdfa}.
% \begin{figure}[H]
%       \centering
\begin{center}
      \resizebox{0.7\textwidth}{!}{
...
%   \end{figure}
Consider $G'$ in Figure~\ref{fig:intro:dangling_edge}. $G'$ is obtained from $G$ in Figure~\ref{fig:preliminaries:a_rewriting_step} by adding an edge from node $3$ to node $6$ (shown in red). This graph has the occurrence \raisebox{2pt}{
            \scalebox{0.7}{\tikz[baseline=-0.5ex]{
            \node [draw,circle] (z) at (-1,0) {$1$};
            \node [draw,circle,orange] (x) at (0,0) {$3$};
            \node[draw,circle] (y) at (1,0) {$2$};
...
\]
in \(\mathbf{Graph}\) and the match \(L \overset{m}{\rightarrow} G\) shown
below.
%  in Figure Figure~\ref{fig:preliminaries:a_match_dangling_edge_condition}. 
 The rule deletes a node of the graph, and the match identifies the three nodes of \(L\) with a single node of \(G\).
    % \begin{figure}[H]
    %     \centering
    \begin{center}
        \resizebox{0.9\textwidth}{!}{
...
Hereafter, \(\mathfrak{M}\) denotes the DPO rewriting framework that associates each rule with the class of all DPO diagrams of the form shown in Figure~\ref{fig:preliminaries:a_rewriting_framework} with monic match $m$.
