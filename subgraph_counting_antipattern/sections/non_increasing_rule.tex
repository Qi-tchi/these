% We use \( S \uplus S' \) to denote the disjoint union of sets \( S \) and \( S' \). Let \( \rho \mathop{=} (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R) \) be a rule.
% \newline
% \noindent
%     \begin{minipage}{0.65\textwidth}
%       The DPO diagram for \( G \mathop{\Rightarrow}_\rho H \) with injective match \( m \) can be seen, up to graph renaming, as a diagram in the category of sets where all morphisms are inclusions. The diagram is shown on the right.
%     \end{minipage}
%     \hfill
...
%         }
%     \end{minipage}
% \( L', R', C', K \) are mutually disjoint sets such that \( L' \uplus K \mathop{=} L \), \( R' \uplus K \mathop{=} R \), and \( C' \uplus K \mathop{=} C \). 
% Additionally, \( G \mathop{=} L \mathop{\cup} C \) and \( H \mathop{=} R \mathop{\cup} C' \). Thus, any subgraph of \( H \) (resp. \( G \)) can be seen as a union of subgraphs of \( C \) and \( R \) (resp. \( L \)).
...
% %     For such an occurrence \( x \), let \( R' \mathop{=} x \mathop{\cap} R \), \( C' \mathop{=} x \mathop{\cap} C \), and let \( h_{R'L}: R' \rightarrowtail L \) be a monomorphism preserving interface elements. The \textbf{corresponding \( X \)-occurrence} relative to \( h_{R'L} \) is \( h_{R'L}(R') \mathop{\cup} C' \).  
...
%     % The set \(D(R,X)\) consists of all subgraphs \( R' \mathop{\subseteq} R \) such that there exists a diagram (shown on the right) where
%     % \(h_{R'X} \mathop{\colon} R' \rightarrowtail X \) and \(h_{C'X} \mathop{\colon} C' \rightarrowtail X \) are not isomorphisms, and \(h_{K'K} \mathop{\colon} K' \rightarrowtail K \) and \(h_{R'R} \mathop{\colon} R' \rightarrowtail R \) are inclusion functions. The union of all subgraphs in \( D(R,X) \) is denoted \( R_X \).
%     % Let \(X\) be a graph and 
%     % \(
%     %     L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R
%     % \) an injective DPO rewriting rule.
%     % %  such that $l$ is edge-injective and $r$ is node-injective.
%     % The set \(D(R,X)\) consists of all subgraphs \( R' \mathop{\subseteq} R \) such that 
%     % we can construct a diagram as shown on the right where \(h_{K'K} \mathop{\colon} K' \rightarrowtail K \) and \(h_{R'R} \mathop{\colon} R' \rightarrowtail R \) are inclusion functions, and \(h_{R'X} \mathop{\colon} R' \rightarrowtail X \) and \(h_{C'X} \mathop{\colon} C' \rightarrowtail X \) are not isomorphisms. The union of all subgraphs in \( D(R,X) \) is denoted \( R_X \).
%     Let \(\mathcal{X}\) be a ruler-graph with underlying graph \(X\) and
%     \(
%         L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R
%     \) an injective DPO rewriting rule.
%     %  such that $l$ is edge-injective and $r$ is node-injective.
%     The set \textbf{\(D(R,X)\)} consists of all subgraphs \( R' \mathop{\subseteq} R \)
...
%         \item  \label{def:drx:rp_diagram} a diagram as shown on the right where \(h_{K'K} \mathop{\colon} K' \rightarrowtail K \) and \(h_{R'R} \mathop{\colon} R' \rightarrowtail R \) are inclusion functions can be constructed.
%     \end{itemize}
%     \end{minipage}
...
% The first condition of the definition ensures that any $R' \mathop{\in} D(R,X)$ can be glued along some common interface elements with a subgraph $C'$ of a context graph \( C \) to form an \( X \)-occurrence. The second and third conditions ensure that such $X$-occurrences are included neither in \( C \) nor in \( R \), thereby guaranteeing that they are implicitly created as defined in Definition~\ref{def:corresponding_occurrence}.
...
%     $R'$ is neither the empty graph nor isomorphic to $X$, by Conditions 2 and 3 of Definition~\ref{def:rx}. 
%     Therefore, $R'$ must contain an interface node (either node 1 or node 2), because if it contains no interface node, Condition 1 of Definition~\ref{def:rx} would be violated due to $X$'s connectivity.
...
%         }
%     \end{center}
%     Let $R' \mathop{\in} D(R,X)$. $R'$ must contain node 3, otherwise it would be a subgraph of $r(K)$, violating Condition 2 of Definition~\ref{def:rx}. Node 3 must be connected to an interface node, otherwise Condition 1 of Definition~\ref{def:rx} would be violated. Therefore, $R'$ must include \raisebox{2pt}{\scalebox{0.6}{\tikz[baseline=-0.5ex]{
...
% \end{example}
% % \begin{remark}
% %     Consider a rewriting step defined by the diagram in Definition~\ref{def:rewriting_step}. Let $X$ be a ruler-graph. The set $\operatorname{Mono}(X,H,\lnot m', \lnot r')$ (resp. $\operatorname{Mono}(X,H,\lnot m', \lnot r')$) is the set of all monomorphisms whose image is an occurrence of $X$ in $H$ (resp. $G$) formed by the image of a graph $R' \mathop{\in} D(R,X)$ and the image of a graph $C' \mathop{\subseteq} C$ around the image of a graph $K' \mathop{\subseteq} K$.
% % \end{remark}
% % \begin{definition}
% %    Let $X$ be a ruler-graph. Consider a rewriting step defined by the diagram in Definition~\ref{def:rewriting_step}. An occurrence $h_{XH}:X \mathop{\rightarrow} H$ of $X$ in $H$ is a \textbf{created occurrence} of $X$ in $H$ if the image of $h_{XH}$ is neither fully include in $m'(R)$ nor fully include in $r'(C)$. For a created occurrence $h_{XH}$, let $R' \mathop{\subseteq} R$ such that $m'(R') \mathop{=} h_{XH}(X) \mathop{\cap} m'(R)$, and let $h_{R'L}: R' \rightarrowtail L$ be a monomorphism, we say that an $h_{XG}$ 
% % \end{definition}
...
% %     \noindent
% %     \begin{minipage}{0.7\textwidth}\setlength{\parindent}{1em}
% %     Let \(\rho \mathop{=} (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R)\) be a rule, and let $X$ be a graph.
...
% %     \end{minipage}
% %      \begin{enumerate}
% %         \item[(1)] \label{def:non_increasing:cond_1} For all $R' \mathop{\in} D(R,X)$ the diagram displayed above on the right holds, where $h_{R'R}$ is the inclusion function;
% %         \item[(2)] \label{def:non_increasing:non_clapse} For all $R' \mathop{\in} D(R,X)$ and for all nodes and arrows $x$ in $R'$, if $ x \notin \operatorname{Im}(r)$ then $\Psi(R')(x) \notin \operatorname{Im}(l)$;
...
%         % For all $R' \mathop{\in} D(R,X)$, there exists a morphism \(h_{R'L}: R' \rightarrowtail L \) such that \trackedtext{both squares in the diagram shown on the right are pullbacks}, where all morphisms other than $h_{R'L}$, $l$ and $r$ are inclusion functions,
...
\begin{example}
    ...
    in Example~\ref{ex:grsaa_rx}. Let $X$ be \tikz[baseline=-0.5ex]{
        \node (x) at (0,0) {$\bullet$};
        \node (y) at (1,0) {$\bullet$};
        \node (z) at (2,0) {$\bullet$};
        \draw[->] (x) -- (y) node[midway, above] {$a$};
    ...
    By Example~\ref{ex:grsaa_rx}, the set \( D(R,X) \) consists of exactly four graphs: $R'_1$, $R'_2$, $R'_3$ and $R'_4$.
    The rule is $X$-non-increasing because the following diagrams can be constructed, and $h_{R'_1L}, h_{R'_2L}, h_{R'_3L}$ and $h_{R'_4L}$ satisfy all conditions in Definition~\ref{def:creates_more_x_on_the_left}. 
    %1
    \begin{center}
        \resizebox{0.7\textwidth}{!}{
            \begin{tikzpicture}
    ...
    %         }
    %     \end{center}
    % To illustrate the property captured by Definition~\ref{def:creates_more_x_on_the_left},
    % consider the rewriting step defined by the DPO diagram below. Whenever an element $R' \mathop{\in} D(R,X)$ forms an \( X \)-occurrence in $H$ that is included neither in \( R \) nor $C$ with $C' \mathop{\subseteq} C$, the graph
...
The second condition guarantees that any $X$-occurrence implicitly created by the rewriting step has its corresponding $X$-occurrence not included in the context (Consequently, its corresponding $X$-occurrence is implicitly destroyed by the rewriting step). Counterexamples are provided in Example~\ref{ex:cond_2_necessaire} and Example~\ref{ex:cond_2_necessaire2}.  
...
\begin{example} 
    ...
    \end{center}
    As shown in Example~\ref{ex:rx_counter_ex}, \( D(R,X) \) contains the graph $R'$:
    \raisebox{2pt}{
...
\begin{example}
    ...
    Despite the existence of a unique monomorphism $h_{R'L}: R' \rightarrowtail L$ preserving interface elements, this rule fails the second condition of Definition~\ref{def:creates_more_x_on_the_left}.
    For any rewriting step using this rule, the corresponding $X$-occurrence relative to $h_{R'L}$ of any implicitly created $X$-occurrence is included in the context.
\end{example}
\begin{example}
    ...
    Despite unique monomorphisms \( h_{R'L}: R' \rightarrowtail L \) and \( h_{R''L}: R'' \rightarrowtail L \) preserving interface elements, they fail the third condition of Definition~\ref{def:creates_more_x_on_the_left}. 
...
\begin{example}
    ...
    There are unique monomorphisms $h_{R'L}:R' \rightarrowtail L$ and $h_{R''L}:R'' \rightarrowtail L$ preserving interface elements, but they fail the fourth condition of Definition~\ref{def:creates_more_x_on_the_left}.
    For any rewriting step using this rule, implicitly created $X$-occurrences with the same subgraph of the context have the same corresponding $X$-occurrence.
\end{example}