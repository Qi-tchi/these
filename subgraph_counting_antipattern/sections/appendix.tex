...
%    By Definition~\ref{def:weight_excluding},
    ...
    By Lemma~\ref{lem:1_ex} and Lemma~\ref{lem:1_dist}, there exists an injective function $\mathbb{F} : \operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'}) \to \operatorname{Hom}(X, G, \lnot m, \lnot l')$. Hence, $|\operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})| \leq |\operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})|$.
   
    By Lemma~\ref{lem:2_exist} and Lemma~\ref{lem:2_distinct}, there exists an injective function $\mathbb{G} : \operatorname{Hom}(X, H, \lnot m'', \lnot r') \to \operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})$. Thus, $|\operatorname{Hom}(X, H, \lnot m'', \lnot r')| \leq |\operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})|$.
...
% \begin{proof}
%     Suppose, by contradiction, that there exist \(b \in B \setminus \operatorname{Im}(\alpha)\) and \(c \in C\) such that \(\beta'(b) = \alpha'(c)\). By Lemma~\ref{lem:b_c_same_img_exist_a}, there exists \(a \in A\) such that \(\alpha(a) = b\). This contradicts the assumption that \(b \notin \operatorname{Im}(\alpha)\) since \(\alpha(a) = b\) implies \(b \in \operatorname{Im}(\alpha)\).  
% \end{proof}
...
% \end{lemma}
% \begin{proof}
%     we have $ h_{K'C'} \star h_{C'C} \star h_{CH'} = h_{K'R'} \star h_{R'R_X} \star h_{R_XH'} $ by Lemma~\ref{lem:dpo_dpo_unique_homo}. By the universal mapping property of the pushout $\square K'C'XR'$, we conclude.
%     \qed
% \end{proof}
...
\begin{proof}
   ...
   Given the monicity of \(h_{KC}\) and Equation~\eqref{lem:nincl:hyp1}, we can define a graph homomorphism \(h_{C'K}\) such that for any \(c' \in C'\), we have \(h_{C'K}(c') \overset{\operatorname{def}}{=} k \in K\), where \(k\) is the unique element in \(K\) satisfying \(h_{KC}(k) = h_{C'C}(c')\). We then have:
\begin{flalign*}
...
\begin{flalign*}
    ...
    =& h_{K'C'} \star  h_{C'C}  \star h_{CH'} & \text{by Equation~\eqref{eq:h_cpk_star_h_kc_eq_h_cpc}} \\
...
Thus, the commutative diagram depicted below holds:
    ...
    \begin{flalign*}
        ...
        =  & (h_{C'K} \star h_{KR_X}) \star h_{R_XH'}  & \text{by Equation~\eqref{hcpx_star_hxrx}}\\
        ...
        =  & h_{C'C} \star  h_{CH'}  & \text{by Equation~\eqref{eq:h_cpk_star_h_kc_eq_h_cpc}}\\
    \end{flalign*}
    and 
    \begin{flalign*}
        & h_{R'X} \star (h_{XR_X} \star  h_{R_XH'})    \\
        = &  (h_{R'X} \star h_{XR_X}) \star  h_{R_XH'} & \text{by associativity}  \\
        =&  h_{R'R_X} \star  h_{R_XH'}  & \text{by Equation~\eqref{hrpx_star_hxrx}}  \\
    \end{flalign*}
...
\begin{proof}
    ...
    By Lemma~\ref{lem:nincl}, we have \( \operatorname{Im}(h_{C'C}) \setminus \operatorname{Im}(h_{KC}) \neq \emptyset \). 
    ...
    \begin{itemize}
        ...
        \end{flalign*}
        By Equation~\eqref{eqimpossible} and Lemma~\ref{lem:b_c_same_img_exist_a}, there exists \( k \in K \) such that the equations \( h_{KC}(k) = c \).
...
\begin{proof}
    ...
    From Equation~\eqref{m1} and \todo{to debug !!! removed : condidion for all $a \in R_X$, we have $h_{R_XL}(a) \in \operatorname{Im}(h_{KL})$ if and only if $a \in \operatorname{Im}(h_{KR_X})$ in the definition of $R_X$} \textcolor{red}{the edge-injectivity} \textcolor{blue}{$h_{R_XL}$ do not identify interface nodes with non-interface nodes} , we have 
    \begin{flalign*}
...
\end{center}
    ...
    \begin{flalign*}
        ...
        &= (h_{R'X} \star (h_{XC} \star h_{CG}))(r') & \text{by Equation~\eqref{hyp:xhpgc}} \\
    ...
    \end{center}
        ...
        \begin{flalign*}
            ...
            =& (h_{C'X} \star (h_{XL} \star h_{LG}))(c') & \text{by Equation~\eqref{hyp:xhpgl}} \\
...
\begin{proof}
    ...
    We have $h_{XH'} \star h_{H'G} \in \operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})$ by Lemma~\ref{lem:hxhp_star_hhpg_in_psi_u_lnotl_lnotc}\todo{ici, we do not need c' and r' be arrows}.
...
\begin{proof}
    ...
    \begin{center}
        ...
        \begin{flalign*}
            ...
            &= (h_{R'X} \star h_{XH'}) \star h_{H'H} &\text{by Equation~\eqref{eq:rpxhprx}}\\
...
\begin{itemize}
    ...
    \begin{flalign*}
        ...
        &= h_{XH'} \star h_{H'H} & \text{by Equation~\eqref{hyp:xchp}} \\
...
\begin{itemize}
    ...
    \begin{flalign*}
        h_{XR} \star h_{RH} &= (h_{XR_X} \star h_{R_XR}) \star h_{RH} & \text{by definition of $h_{XR}$} \\
                            ...
                            &= h_{XH'} \star h_{H'H} & \text{by Equation~\eqref{hyp:xrxhp}} \\
                            &= h_{XH} & \text{by commutativity of $\triangle XHH'$}
    \end{flalign*}
...
\begin{proof}
    ...
    By Lemma~\ref{lem:exist_hxhp}, there is a homomorphism $h_{XH'}$ such that $h_{XH'} \star h_{H'H} = h_{XH}$.
    ...
    We conclude that $h_{XH'} \in \operatorname{Hom}(X, H', \lnot h_{R_XH'}, \lnot h_{CH'})$ by Lemma~\ref{lem:ifin_psi_lnot_r_lnot_c_thenin_psi_lnot_rx_lnot_c}.
