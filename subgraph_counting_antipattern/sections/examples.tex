\begin{example}
  ...
  The rewriting rule illustrated in~\autoref{fig:antipattern:def:contrib_variant} modifies a rule from~\cite[Example 6]{plump2018modular} by removing all interface edges and adding a node with identifier $5$ and three self-loops labeled $0$.
  \begin{figure}[H]
        \centering
      \resizebox{0.6\textwidth}{!}{
        ...
        \caption{}
        \label{fig:antipattern:def:contrib_variant}
  \end{figure}
...
\begin{example}
  ...
  Consider the rule $\rho$ shown in~\autoref{fig:antipattern:grs_aca} from~\cite[Example 1]{bruggink2014termination}.
  \begin{figure}[H]
    \centering  
...
\card{\operatorname{Mono}(X,R)_{\operatorname{NF}}} = 1 - 0 - 0 = 1 > 0$. The rule terminates by~\autoref{antipattern:thm:termination_grs}.
\todo{check} 
\end{example} 
% Finally, we present a limitation of our approach.
\begin{example}
    \label{antipattern:ex:endrullis:d3:termination}  
    Our method can prove termination of the rewriting rule $\rho$ shown in~\autoref{fig:antipattern:ex_endrullis_d3} from~\cite[Example D.3]{endrullis2024generalized_arxiv_v2}.
    \begin{figure}[H]
        \centering
        \resizebox{0.7\textwidth}{!}{
    \begin{tikzpicture}  
   ...
   \end{tikzpicture}
        ...
        \caption{}
        \label{fig:antipattern:ex_endrullis_d3}
    \end{figure}
    ...
    = 1 - 0 - 0 = 1 > 0$. The rule terminates by~\autoref{antipattern:thm:termination_grs}.
  \end{example} 
