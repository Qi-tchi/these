\begin{example}
  ...
  The rewriting rule illustrated below modifies a rule from~\cite[Example 6]{plump2018modular} by removing all edges of the interface graph and adding a node with identifier $5$ and three self-loops labeled $0$ in the right-hand side graph.
  \begin{center}
    ...
    %   Since $F_X = \emptyset$, the number of $X$-occurrences not included in any occurrences of forbidden context in any rewriting step is predictable.
      By Theorem~\ref{antipattern:thm:termination_grs}, the rule terminates because \(|Mono(X,L)| - |Mono(X,R)| = 1 - 0 = 1 > 0 \). 
\end{example}
\begin{example}
...
\card{\operatorname{Mono}(X,R)_{\operatorname{NF}}} = 1 - 0 - 0 = 1 > 0$. The rule terminates by Theorem~\ref{antipattern:thm:termination_grs}.
\end{example} 
...
\begin{example}
   ...
   \end{tikzpicture}
    ...
    = 1 - 0 - 0 = 1 > 0$. The rule terminates by Theorem~\ref{antipattern:thm:termination_grs}.
  \end{example} 
