...
\begin{definition}
    ...
    Define $\Gamma(\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}})$ as the set of $X$-occurrences in $L$ whose images are not included in any occurrence of the forbidden context in $L$,
    ...
    \begin{itemize}
        \item there does not exist $h_{FL}:F \rightarrowtail L$ such that $f \star h_{FL} = h_{XL}$,
         ...
         there exist a monomorphism $h_{FG} : F \rightarrowtail G$ such that $h_{XL} \star h_{LG} = h_{XF} \star h_{FG}$. 
