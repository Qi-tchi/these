...
We define the following sets of monomorphisms to facilitate the discussion of the termination criterion. 
They differ from those defined in Notation~\ref{subgraph_counting:notation:mono_sets} in that the first argument is now a generalized ruler-graph.
...
Under the assumptions of the above two lemmas, by~\eqref{eq:lkdfjasnveennnaa}, we obtain the following inequality:
 ...
 The following definition introduces a subset of $\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}$ that consists of occurrences of $X$ whose images are not included in any occurrence of the forbidden context in $L$,
    but in some rewriting step $G \mathop{\Rightarrow}_\rho^m H$ their images may possibly be included in an occurrence of the forbidden context in the host graph $G$.
\begin{definition}
    ...
    Define $\Gamma(\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}})$\index{$\Gamma(\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}})$} as the subset of $\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}$ given by the following conditions:
    \begin{itemize}
...
\end{definition}
To improve readability of the subsequent discussion, we define $$\Lambda(\mathcal{X},\rho) \overset{\operatorname{def}}{=} (\card{\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}} \mathop{-} 
    \card{\Gamma(\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}})}) -
   \card{\operatorname{Mono}(\mathcal{X},R)_{\operatorname{NF}}}.$$\index{$\Lambda(\mathcal{X},\rho)$}
...
\begin{theorem}[Termination] 
    ...
    \begin{enumerate}
        ...
        $,
    \end{enumerate}
    then \(\mathop{\Rightarrow}_{\mathcal{A},\mathfrak{M}}\) terminates relative to \(\mathop{\Rightarrow}_{\mathcal{B},\mathfrak{M}}\).
\end{theorem}
\begin{remark}
