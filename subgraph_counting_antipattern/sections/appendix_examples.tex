...
%       \draw[->] (x) to (y);
%   }.
%   We have $|\operatorname{Mono}(X,L)| \mathop{=} 1 \mathop{>} 0 \mathop{=} |\operatorname{Mono}(X,R)|$ for both rules. Thus, this rewriting systems terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}
...
%   %     \label{fig:overbeek}
%   % \end{figure}
%   The bottom span of the diagram is a rewriting rule presented in~\cite[Example 5.5]{overbeek2024termination_lmcs}. Its termination in the DPO rewriting framework with monic matches can be proved by our approach using the same argument for Example~\ref{ex_endrullis_6d3_endrullis_5d8}. 
% \end{example}
...
%   \begin{center}
%     $\rho' \mathop{=} $\scalebox{0.9} { \input{tikz/OE_ex5d6_rho_bis.tex}}
%   \end{center}
...
%   We have 
%   $|\operatorname{Mono}(X,L)| \mathop{=} 1 \mathop{>} 0 \mathop{=} |\operatorname{Mono}(X,R)|$ for $\rho$, and 
%   $|\operatorname{Mono}(X,L)| \mathop{=} 0 \mathop{=} 0 \mathop{=} |\operatorname{Mono}(X,R)|$ for $\tau$, therefore $\rho$ is eliminated.
...
%   and $|\operatorname{Mono}(Y,L)| \mathop{=} 1 \mathop{>} 0 \mathop{=} |\operatorname{Mono}(Y,R)|$.

%   Thus, this rewriting systems terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}
...
%   % \begin{figure}[H] 
%   \begin{center}
%     $\tau \mathop{=} ${ \resizebox{0.6\textwidth}{!}{
...
%   } and $\mathbb{X} \mathop{=} \{X\}$. Let $s_\mathbb{X}$ be the weight function associating the weight of $1$ to $X$. There is only one element 
...
%       $, the rule terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}
...
%   Consider the rewriting rule in Example~\ref{ex:grsaa_rx}. Let $X$ be the graph \tikz[baseline=-0.5ex]{
%       \node (x) at (0,0) {$\bullet$};
%       \node (y) at (1,0) {$\bullet$ }; 
%       \node (z) at (2,0) { $\bullet$};
%       \draw[->] (x) -- (y) node[midway, above] {$a$};
%       \draw[<-] (z) -- (y) node[midway, above] {$a$};
%   } with weight $1$ and $\mathbb{X} \mathop{=} \{X\}$. The rule is $X$-non-increasing by Example~\ref{example:grs_aa:has_more_left}. 
%   % Let $s_\mathbb{X}$ be the weight function which associates the weight of $1$ to $X$. 
%   % Since \(w_{s_\mathbb{X}}(L) \mathop{=} 1 \mathop{>} 0 \mathop{=} w_{s_\mathbb{X}}(R)\), 
%   Since \(w(L) \mathop{=} 1 \mathop{>} 0 \mathop{=} w(R)\),
%   it terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}
...
%   \begin{center}
%     $\rho \mathop{=} $\scalebox{0.7} { \input{tikz/OE_ex5d6_rho.tex}}
%   \end{center}
%   \begin{center}
%   $\tau \mathop{=} $\scalebox{0.7}{ \input{tikz/OE_ex5d6_tau.tex}
%   }
%   \end{center}
%   Let $X$ be 
%   \tikz[baseline=-0.5ex]{
...
%   } with weight $3$ and $\mathbb{X} \mathop{=} \{X, Y\}$.
%   The rules $\rho$ and $\tau$ are both $X$- and $Y$-non-increasing with $D(rhs(\rho), X) \mathop{=} D(rhs(\rho), Y) \mathop{=} D(rhs(\tau), X) \mathop{=} D(rhs(\tau), Y) \mathop{=} \emptyset$.   
%   % Let $s_\mathbb{X}$ be the weight function associating the weight of $5$ to $X$ and the weight of $3$ to $Y$.
%   % We have $
%   %     w_{s_\mathbb{X}}(\operatorname{lhs}(\rho)) \mathop{=} 10 \mathop{>} 9 \mathop{=} w_{s_\mathbb{X}}(\operatorname{rhs}(\rho))
%   %     $ and $
%   %     w_{s_\mathbb{X}}(\operatorname{lhs}(\tau)) \mathop{=} 6 \mathop{>} 5 \mathop{=} w_{s_\mathbb{X}}(\operatorname{rhs}(\tau))$.
%   We have $
%   w(\operatorname{lhs}(\rho)) \mathop{=} 10 \mathop{>} 9 \mathop{=} w(\operatorname{rhs}(\rho))
%   $ and $
%   w(\operatorname{lhs}(\tau)) \mathop{=} 6 \mathop{>} 5 \mathop{=} w(\operatorname{rhs}(\tau))$.
%   Thus, this rewriting systems terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}