...
%       \draw[->] (x) to (y);
%   }.
%   We have $|\operatorname{Mono}(X,L)| = 1 > 0 = |\operatorname{Mono}(X,R)|$ for both rules. Thus, this rewriting systems terminates by Theorem~\ref{thm:termination_grs}.
% \end{example}
...
%   \begin{center}
%     $\rho' = $\scalebox{0.9} { \input{tikz/OE_ex5d6_rho_bis.tex}}
%   \end{center}
...
%   } with weight $1$ and $\mathbb{X} = \{X\}$. The rule is $X$-non-increasing by Example~\ref{example:grs_aa:has_more_left}. 
...
%   \begin{center}
%     $\rho = $\scalebox{0.7} { \input{tikz/OE_ex5d6_rho.tex}}
%   \end{center}
%   \begin{center}
%   $\tau = $\scalebox{0.7}{ \input{tikz/OE_ex5d6_tau.tex}
%   }
%   \end{center}
%   Let $X$ be 
%   \tikz[baseline=-0.5ex]{
