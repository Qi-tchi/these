...
\end{lemma}
\begin{proof}
    By~\autoref{def:gamma_l_rho_x} of $\Gamma$.
\end{proof}

\todo{what assumption does \autoref{lem:xgm_xhmp_xl_xr} need?}
\noindent
\begin*{\textbf{\autoref{lem:xgm_xhmp_xl_xr}}}
    \newline
    \noindent
...
\begin{proof}
    ...
    \begin{flalign*}
        \card{\operatorname{Mono}(\mathcal{X},G,m)} - 
        \card{\operatorname{Mono}(\mathcal{X},H,m')} \geq  
        \card{\operatorname{Mono}(\mathcal{X},L)} - 
   \card{\operatorname{Mono}(\mathcal{X},R)}
    \end{flalign*}
    ...
    \begin{flalign*}
        ...
        \\
           ...
           \\ 
           & \text{by definition of $\operatorname{Mono}(\mathcal{X},G,m)_{\operatorname{NF}}$ and $\operatorname{Mono}(\mathcal{X},H,m')_{\operatorname{NF}}$}
    \end{flalign*}
    where 
    $\{
                h_{XL} \in \operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}} \mid 
    ...
    %   )  
        \text{\hspace{1cm}by~\autoref{lem:leq_gamma_mono_x_l}}
      \\
      \mathop{=}& 
      \card{\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}} - 
      \card{
...
\end*{}
\begin{proof}
    \label{proof:lem:xglnotmlp_xhlnotmrp}
   ...
   \end{tikzpicture}
    ...
    \begin{flalign}
        h_{XK} \star u = h_{XC} \label{eq:hxk_u}
   ...
   \end{tikzpicture}
    ...
    Thus, we have \begin{flalign*}
        ...
        &= h_{XK} \star u \star h_{CG} &\text{by~\eqref{eq:hxk_u}}\\
    ...
    and there does not exist $h_{FC}:F \rightarrow C$ such that $h_{FG} = h_{FC} \star l'$.
     By~\autoref{lem:xh_decomp}, we can construct the following commutative diagram:
...
\begin{proof} 
 ...
 $
    ...
    Let us now consider the second case.
     \todo{assumption for~\autoref{lem:w_u_l_not_geq_r_not}: $\rho$ is $X$-non-increasing} The following holds:
    \begin{flalign*}
        ...
        \geq & 
            ...
            \card{\operatorname{Mono}(\mathcal{X},G,\lnot m, \lnot l')_{\operatorname{F}}}
        \\
        & \text{by~\autoref{lem:w_u_l_not_geq_r_not}}
    \end{flalign*}    
    ...
    \begin{flalign}
        h_{XG} = f \star h_{FG} \label{eq:xxxhxg}
    \end{flalign}
    By~\autoref{lem:xh_decomp}, we can construct the following commutative diagram:
...
\begin{enumerate}
    \item\label{thm:termination_grs:assump:1}  for every $\rho \in \mathcal{A} \cup \mathcal{B}$ and for every \( \mathcal{X} \in \mathbb{X} \) with underlying graph $X$, 
    ...
    \item\label{thm:termination_grs:assump:2}  for every \(\rho \in \mathcal{A}\), 
    ...
    \( \sum_{\mathcal{X} \in \mathbb{X}}^{}s_\mathbb{X}(X) * \left( 
        ...
        \right) > 0 \),
    \item\label{thm:termination_grs:assump:3}  for every \(\rho \in \mathcal{B}\),  
