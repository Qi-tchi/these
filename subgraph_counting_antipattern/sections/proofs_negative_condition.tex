...
We have
$$\card{\operatorname{Mono}(\mathcal{X},G,m)_{\operatorname{NF}}} = \card{\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}} - \card{\{
                h_{XL} \in \operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}} \mid 
                \exists h_{FG}. h_{XL} \star m = f \star h_{FG}
            \}}$$ 
$$\card{\operatorname{Mono}(\mathcal{X},H,m')_{\operatorname{NF}}} = \card{\operatorname{Mono}(\mathcal{X},R)_{\operatorname{NF}}} - \card{\{
...
\begin{proof}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[red,dashed,>->] (f) -- (g) node[midway,right] {};
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[>->] (k) -- (c) node[midway, left] {};
            \draw[>->] (x) -- (g) node[midway,below] { };
            \draw[dashed, red,>->] (f) -- (l) node[midway,left] { };
            \draw[>->] (x) -- (l) node[midway] {};
    ...
    It is clear that $h_{XL} \notin \{
                h_{XL} \in \operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}} \mid 
                \exists h_{FG}. h_{XL} \star m = f \star h_{FG}
            \}$ because otherwise, the image of $h_{XG}$ would be included in the image of $f \star h_{FG}$ for some morphism $f$, as shown in the commutative diagram below, which contradicts the assumption that $h_{XG}$ is in $\operatorname{Mono}(\mathcal{X},G,m)_{\operatorname{NF}}$.
    ...
    Conversely, let $h_{XL}$ be a morphism in $\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}\setminus \{
                ...
                \exists h_{FG}. h_{XL} \star m = f \star h_{FG}
            \}$. 
    By definition, $h_{XL} \star m$ is in $\operatorname{Mono}(\mathcal{X},G,m)$. We have thus the following commutative diagram:
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[>->] (l) -- (g) node[midway, right] {m};
            \draw[>->] (k) -- (c) node[midway, left] {};
            \draw[red,dashed,>->] (f) -- (l) node[midway,right] {};
            \draw[>->] (x) -- (l) node[midway,left] { };
            \draw[>->] (x) -- (f) node[midway,left] {f};
            \draw[red,dashed,>->] (f) -- (g) node[midway,right] {};
            \node () [at=($(l)!0.5!(c)$)] {PO};
        \end{tikzpicture}
    \end{center}
    The morphism $h_{XL} \star m$ is in $\operatorname{Mono}(\mathcal{X},G,m)_{\operatorname{NF}}$ because otherwise, there would be a morphism $h_{FG}: F \rightarrowtail G$ such that $f \star h_{FG} = h_{XL} \star m$, which contradicts the assumption that $h_{XL}$ is in $\operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}}\setminus \{
                h_{XL} \in \operatorname{Mono}(\mathcal{X},L)_{\operatorname{NF}} \mid 
                \exists h_{FG}. h_{XL} \star m = f \star h_{FG}
            \}$.
\end{proof}
...
\todo{what assumption does \autoref{antipattern:lem:xgm_xhmp_xl_xr} need?}
\noindent
...
\begin{proof}    
        ...
        Therefore, from the universal property of pullback~\autoref{def:cat:pb} and \(
            h_{K'C'} \star h_{C'C} \star h_{CH} =h_{K'R'} \star h_{R'R} \star h_{R'R}
        \), there exists a unique morphism$h_{K'K}:K' \rightarrowtail K$ such that 
        \begin{flalign}
            h_{K'C'} \star h_{C'C} &= h_{K'K} \star h_{KC} \label{antipattern:kpcpcpckpkkc}
            \\
            h_{K'R'} \star h_{R'R} &= h_{K'K} \star h_{KR} \label{antipattern:kprprprkpk}
        \end{flalign} 
...
\begin*{\textbf{\autoref{lem:xglnotmlp_xhlnotmrp}}}
    ...
    \begin{minipage}{0.7\textwidth}\setlength{\parindent}{1em}
        ...
        % Suppose that the number of $X$-occurrences not included in any occurrences of forbidden context $F \in F_x$ in a $\rho$-rewriting step is predictable. 
        Suppose that $\rho^{-1}$ is $F$-non-increasing.
...
\begin{proof}
    ...
    In the first case, the inequality is equivalent to $
        \card{\operatorname{Mono}(\mathcal{X},G,\lnot m, l')} \geq
        \card{\operatorname{Mono}(\mathcal{X},H,\lnot m', r')}$, because there is no forbidden context. Let $h_{XH} \in \operatorname{Mono}(\mathcal{X},H,\lnot m', r')$. By definition of $\operatorname{Mono}(\mathcal{X},H,\lnot m', r')$, there exists a monomorphism $h_{XC}:X \rightarrowtail C$ such that $h_{XH} = h_{XC} \star r'$. We have $h_{XC} \star l' \in \operatorname{Mono}(\mathcal{X},G,\lnot m, l')$. Furthermore, if $h_{XH}, h_{XH}' \in \operatorname{Mono}(\mathcal{X},H,\lnot m', r')$ with $h_{XH} \neq h_{XH}'$, then there exist $h_{XC}, h_{XC}'$ such that $h_{XH} = h_{XC} \star r'$ and $h_{XH}' = h_{XC}' \star r'$. Since $r'$ is a monomorphism, we deduce that $h_{XC} \neq h_{XC}'$. Therefore, from the injectivity of $l'$, we have $h_{XC} \star l' \neq h_{XC}' \star l'$. Thus, we have $\card{\operatorname{Mono}(\mathcal{X},G,\lnot m, l')} \geq
   ...
   We prove that $\phi(h_{XG})$ cannot be factorized by $m'$, by contradiction. Suppose that $\phi(h_{XG})$ can be factorized by $m'$, i.e. there is $h_{XR}:X \rightarrow R$ such that $\phi(h_{XG}) \overset{def}{=} h_{XC} \star r' = h_{XR} \star m'$. $h_{XR}$ is injective, because $\phi(h_{XG})$ is injective. Then, since a pushout is also a pullback in \textbf{Graph} by~\autoref{prop:pb_eq_po},
   ...
   \end{tikzpicture}
    ...
    By the universal property of pullback, there exists a unique morphism $h_{XK}:X \rightarrowtail K$, which is injective because $h_{XR}$ is injective, such that $h_{XK} \star r = h_{XR}$ and 
    \begin{flalign}
        h_{XK} \star u = h_{XC} \label{antipattern:eq:hxk_u}
    \end{flalign} Therefore, the following commutative diagram holds:
   ...
   \end{tikzpicture}
    ...
    Thus, we have \begin{flalign*}
        h_{XG} &= h_{XC} \star h_{CG} \\
        &= h_{XK} \star u \star h_{CG} &\text{by Equation~\ref{antipattern:eq:hxk_u}}\\
        &= h_{XK} \star h_{KL} \star m
    \end{flalign*}
    Which is a contradiction because $h_{XG}$ is an element of $\operatorname{Mono}(\mathcal{X},G,\lnot m, l')_{\operatorname{F}}$ and, by definition, cannot be factorized by $m$. Thus, $\phi(h_{XG})$ cannot be factorized by $m'$.

    In order to show that $\phi(h_{XG})$ is an element in $\operatorname{Mono}(\mathcal{X},H,\lnot m', r')_{\operatorname{F}}$, since $\phi(h_{XG})$ can be factorized by $r'$ by definition, it remains to show that it can be decomposed as $x \star y$ with $x: X \rightarrowtail F$ and $y: F \rightarrowtail H$.

    There are three mutually exclusive cases according to whether or not the morphism $h_{FG}$ can be factorized by $m$ or $l'$:
    ...
    \begin{itemize}
        ...
        Therefore, we would have $h_{XG} = h_{XF} \star h_{FL} \star m$, which would imply that $h_{XC}$ is not in $\operatorname{Mono}(X,G,\lnot m, l')$.
    ...
    By~\autoref{lem:g_monic}, there exists a unique monomorphism $h_{FH}:F \rightarrowtail H$ such that $h_{L'R} \star h_{RH} = h_{L'F} \star h_{FH}$ and $h_{C'F} \star h_{FH} = h_{C'C} \star h_{CH}$, because $K'C'FL'$ is a pushout, and 
    \begin{flalign*}
    ...
    \begin{center}
        ...
        \end{center} 
        where $h_{L'R}$ is given by the $F$-non-increasing property of $\rho^{-1}$.\todo{assumption: the $F$-non-increasing property of $\rho^{-1}$}
...
\begin*{\textbf{\autoref{lem:xglnotmlnotlp_xhlnotmrnotrp}}}
    ...
    \vspace{2mm}
    \begin{minipage}{0.7\textwidth}\setlength{\parindent}{1em}
        Let $\mathcal{X}$ be a ruler-graph with underlying graph $X$ and \( \rho = (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R) \) be a rule. Suppose that $\rho$ and $\rho^{-1}$ are $X$-non-increasing.
        Consider the DPO diagram illustrated on the right. We have 
    \end{minipage}%
    \begin{minipage}{0.29\textwidth}
        \hfill
...
\begin{proof} 
 ...
 $
    ...
    Let us now consider the second case.
     \todo{assumption for~\autoref{subgraph_counting:lem:w_u_l_not_geq_r_not}: $\rho$ is $X$-non-increasing} The following holds:
    \begin{flalign*}
    ...
    Let $h_{XG} \in \operatorname{Mono}(\mathcal{X},G,\lnot m, \lnot l')_{\operatorname{F}}$. By definition, the following holds:
    \begin{flalign}
    ...
    \begin{center}
        ...
        There exists a monomorphism $h_{L'R} : L' \rightarrowtail R$ because $\rho^{-1}$ is $X$-non-increasing by assumption \todo{assmption: $\rho^{-1}$ is $X$-non-increasing} and we can construct the following commutative diagram by~\autoref{lem:g_monic}:
        ...
        by $\phi(h_{XG}) = h_{XH}$. Since $\phi$ is injective, by~\autoref{lem:h_hp_diff_g_gp_diff} \todo{~\autoref{lem:h_hp_diff_g_gp_diff} uses the assumption: $\rho$ is $X$-non-increasing}, it remains to show that $\phi$ is well defined which requires to show that $\phi(h_{XG})$ is an element in $\operatorname{Mono}(X,H,\lnot m', \lnot r')_{\operatorname{NF}}$, which is equivalent to show that:
        \begin{enumerate}
        ...
        If $h_{XH}$ can be factorized by $m'$, then there exists a unique monomorphism $h_{XL'}:X \rightarrowtail L'$ such that $Id_X = h_{XL'} \star h_{L'X}$ and $h_{XR} = h_{XL'} \star h_{L'R}$ because $L"XHR$ is a pullback\todo{detail maybe//urgence faible}.
        Then \begin{flalign*}
    ...
    because $L''LGF$ is a pullback, and the following diagram hodls 
