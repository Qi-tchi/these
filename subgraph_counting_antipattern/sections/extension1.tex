...
\begin{proof}
    \begin{flalign}
        ...
        &(|Mono(X+ F, H, \lnot r', m')| - |Mono(X+ F, G, \lnot l', m)|) \\
        &\text{by Lemma~\ref{lem:decomp_w_u} and Lemma~\ref{lem:xlnlmxrnr} } \\
    \end{flalign}
...
\begin{flalign*}
   ...
   \mathop{\geq} & (\card{Mono(X, L)} - \card{Mono(X, R)})+(\card{Mono(X+ \mathbb{F}, H)} - \card{Mono(X+ \mathbb{F}, G)}) & \text{by Lemma~\ref{lem:w_g_geq_w_h_leq}}\\
\end{flalign*}
