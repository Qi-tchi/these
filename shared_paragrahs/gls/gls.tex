...
%   Let \( \rho \mathop{=} ((U,\lambda),(U, \lambda')) \) be a graph relabeling rule.
...
%   Let \( \rho \mathop{=} L \mathop{\to} R \) be a graph relabeling rule. Let \( (G, \lambda_G), (G, \lambda'_G) \) be a labeled graph.
%   Let \( m : L \mathop{\to} (G,\lambda_G) \) be a match of \( \rho \) in \( (G, \lambda_G) \).
%   A couple of labeled graph morphisms $(m, m': R \rightarrowtail (G, \lambda'_G))$ such that $m(x) \mathop{=} m'(x)$ for all nodes and edges $x$ in $G$ is witness a \textbf{graph relabeling step} from $(G, \lambda_G)$ to $(G, \lambda'_G)$ using the rule $\rho$ and match $m$, and we denote $(G,\lambda) \mathop{\Rightarrow}_{\rho,\mathfrak{gls}}^m (G,\lambda')$.
% \end{definition}
...
%   Let \( \rho \mathop{=} (L, \lambda_L) \mathop{\to} (R,\lambda_R) \) be a graph relabeling rule, \( (G, \lambda_G) \) a labeled graph.

%   The witness of rewriting steps 
%   $(m: (L, \lambda_L) \rightarrowtail (G,\lambda_G), m': (R, \lambda_R) \rightarrowtail (G, \lambda'_G))$ defines a \textbf{graph rewriting step} from \( (G, \lambda_G) \) to \( (G, \lambda'_G) \) using the rule \( \rho \) and match \( m \), and we denote $(G,\lambda_G) \mathop{\Rightarrow}_{\rho,\mathfrak{gls}}^m (G,\lambda'_G)$.
% \end{definition}

% \begin{definition}[Graph Relabeling Step \cite{litovsky1999graph}]
%   \label{def:graph_relabeling_step}
%   Let $(G,\lambda_G), (G, \lambda_G')$ be two labeled graphs sharing a common unlabeled graph, $ \rho \mathop{=} ((U,\lambda),(U, \lambda')) $ a graph relabeling rule and $m$ a match of $\rho$ in $(G,\lambda_G)$. We say that there is a \textbf{graph-relabeling-step} from $(G,\lambda_G)$ to $(G,\lambda_G')$ with rule $\rho$ and match $m$ if the underlying unlabeled graph homomorphism $m$ is also a graph homomorphism from $(R,\lambda_R')$ to $(G,\lambda_G')$, and we denote $(G,\lambda) \mathop{\Rightarrow}_{\rho,\mathfrak{gls}}^m (G,\lambda')$.
% \end{definition}

% \begin{definition}[Graph Relabeling System]
%   The \textbf{relabeling relation $\mathop{\Rightarrow}_{\mathcal{R},\mathfrak{gls}}$ induced by a set $\mathcal{R}$ of relabeling rules} is given by: $G \mathop{\Rightarrow}_{\mathcal{R},\mathfrak{gls}} H$ iff $G \mathop{\Rightarrow}^m_{\rho,\mathfrak{gls}} H$ for some $\rho \mathop{\in} \mathcal{R}$ and for some match $m$ of $\rho$ in $G$.
% \end{definition}
...
% \begin{definition}[graph-relabeling-step \cite{litovsky1999graph}]
%   Let $H, H'$ be two labeled graphs sharing a common unlabeled graph, $r \mathop{=} (G \mathop{\to} G')$ a graph relabeling rule and $m: G \mathop{\to} H$ a match of the rule $r$ in $H$. We say that there is a \textbf{graph-relabeling-step} from $H$ to $H'$ with rule $r$ and match $m$ if $m$, considered as a function from $G'$ to $H'$, is a labeled graph isomorphism , and we denote $H \mathop{\to} _r^m H'$.
% \end{definition}
% \begin{definition}[Graph-relabeling-systems \cite{litovsky1999graph}]
%  Let $\mathcal{R}$ be a set of graph relabeling rules. For all labeled graph $H$, for all $r \mathop{=} G \mathop{\to} G' \mathop{\in} \mathcal{R}$, let 
%  $$\mathfrak{F}_{gls}(r) \overset{def}{=} \set{(H,H')\in \mathbf{Graph^2} \mathop{\mid} \exists m. H \mathop{\to} _r^m H'}$$ 
%  The rewriting system $(\textbf{Graph}, \mathop{\to} _\mathcal{R})$, générated by $\mathcal{R}$ in $\mathfrak{F}_{gls}$ is called a \textbf{graph relabeling system (GLS)}.

%   % graph relabelling system (GRS) is a triple $\mathcal{R} \mathop{=} (L,I,P)$ where L is a set of labels,
