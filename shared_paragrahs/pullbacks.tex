% \begin{definition}[Pullback \cite{barr1990category, overbeek2023graph}]
%     \label{def:cat:pb}
%     \begin{figure}[htbp] 
%         \center
%             \begin{tikzpicture}
...
%             \end{tikzpicture}
%             \caption{pullback of $(\beta',\alpha')$}
%             \label{diag:def_pb}
%     \end{figure}

 
%     In a category, a \textbf{pullback} of a cospan \(B \overset{\beta'}{\rightarrow} D \overset{\alpha'}{\leftarrow} C \), depicted in~\autoref{diag:def_pb}, is defined as a span \( B \overset{\alpha}{\leftarrow} A \overset{\beta}{\rightarrow} C \) such that for every span \( B \overset{\gamma'}{\leftarrow} E \overset{\gamma}{\rightarrow} C \) if \(\gamma' \mathop{\star} \beta' \mathop{=} \gamma \mathop{\star} \alpha'\) holds, then there exists a unique morphism \(\delta: E \mathop{\to} A\) such that $\gamma' \mathop{=} \delta \mathop{\star} \alpha$ and $\gamma \mathop{=} \delta \mathop{\star} \beta$. The diagram involving \(\alpha\), \(\beta\), \(\alpha'\), and \(\beta'\) is referred to as the \textbf{pullback square}, \(A\) as the \textbf{pullback object}, and the existence of the unique morphism is known as the \textbf{universal mapping property (UMP) of the pullback}.
% \end{definition} 
