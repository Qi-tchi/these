...
For example, the term rewriting rule $f(a,b) \to f(b,a)$ is analogous to the graph rewriting rule in Example~\ref{example:preliminaries:dggdfgnkfsjksfkjsf}:
\vspace{-2mm}
 \begin{center}
            \resizebox{\textwidth}{!}{
                \begin{tikzpicture}
...
% An extension of the weighted type graph method which facilitates searching for suitable type graphs will be presented in Chapter~\ref{chap:nwf}.

The subgraph-counting method weigh graphs by summing weights of morphisms from some designated graphs targeting them. This method, developed by Overbeek and Endrullis~\cite{overbeek2024termination_lmcs} is designed for the PBPO+~\cite{overbeek2023graph, overbeek2023apbpotutorial}\textemdash{}a rewriting formalism capable of simulating left-injective DPO rewriting.
 Additionally, this method can be applied for many different graph notions. 
%  A uncomparable but related approach based on counting subgraphs will be presented in Chapter~\ref{chap:subgraph_counting}.

Plump~\cite{plump1995ontermination} gives a necessary and sufficient termination criterion for left-injective DPO rewriting rules in terms of forward closure which are derivations satisfying certain property. A rewriting system is terminating if and only if it does not admit an infinite forward closure and for each rule $L \overset{l}{\leftarrow} K \overset{r}{\rightarrow} R$, $l$ is not surjective.
However, whether a rewriting system admits an infinite forward closure is undecidable, since this criterion is equivalent to termination of systems with non-surjective left morphisms.
