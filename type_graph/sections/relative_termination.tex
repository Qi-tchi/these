...
\begin{example}
    Consider the rewriting rules in Figure~\ref{fig:preliminaries:graph_transformation_rule_nonterminating}, which
...
\raisebox{2pt}{
    ...
    A looping rewriting chain using this rule is shown in Figure~\ref{fig:preliminaries:sequence_of_transformation_infinite}, in which the subgraph to be replaced at each transformation step is highlighted in red.
...
For example of a non-terminating rule set, consider the set with the unique rule shown in Figure~\ref{fig:preliminaries:graph_transformation_rule_nonterminating} and the infinite rewriting chain shown in Figure~\ref{fig:preliminaries:sequence_of_transformation_infinite}. 
...
However, in many cases, an interesting property can be proved even if the whole rule set is not terminating. For example, consider the graph transformation system with two rules shown below. Rule $\alpha$ deletes an arbitrary edge labeled by $A$, and rule $\beta$ introduces a fresh node.
\begin{center}
        $\alpha$ = {
