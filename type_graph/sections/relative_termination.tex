...
We say that $R$ is \textbf{terminating relative to}\index{{terminating relative to}}\index{Relative termination!of binary relations} $S$ (or that $R$ \textbf{terminates relative to} $S$) if 
...
\begin{definition}
    \label{termination:def:relative_termination}
     ...
     We say that 
     $\mathcal{R}$ is \textbf{terminating relative to}\index{Relative termination!of rule-sets} $\mathcal{S}$ 
     if 
