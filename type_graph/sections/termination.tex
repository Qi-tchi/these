...
It is crucial to ensure that the weight of any object subject to rewriting is not $0_S$, because \(0_S\) behaves unpredictably in strongly monotonic measurable semirings. For instance, in the natural tropical semiring \((\mathbb{N} \cup \{+\infty\}, \min, +)\), the element \(0_S\) is the greatest element \(+\infty\), while in the natural arctic semirings \((\mathbb{N} \cup \{-\infty\}, \max, +)\), the element \(0_S\) is the smallest element \(-\infty\).
...
\begin{definition}[\cite{endrullis2024generalized_arxiv_v2}]
    ...
    A \textbf{context closure} for $\rho$ and $\mathcal{T}$ in $\mathfrak{F}$ is a morphism $c:L \rightarrow T$ such that for every DPO diagram in $\mathfrak{F}(\rho)$ shown below,
