...
\begin{definition}[\cite{endrullis2024generalized_arxiv_v2}]
    ...
    A \textbf{context closure}\index{Context closure} for $\rho$ and $\mathcal{T}$ in $\mathfrak{F}$ is a morphism $c:L \mathop{\rightarrow} T$ such that for every DPO diagram in $\mathfrak{F}(\rho)$ shown below,
...
\begin{definition}[\cite{endrullis2024generalized_arxiv_v2}]
    ...
    \noindent
    The rule $\rho$ is said to be \textbf{Weakly decreasing}\index{weakly decreasing} with respect to $\mathcal{T}$ in $\mathfrak{F}$ if 
    ...
    \noindent
    The rule $\rho$ is said to be \textbf{uniformly decreasing}\index{Uniformly decreasing} with respect to $\mathcal{T}$ in $\mathfrak{F}$ if the following conditions holds:
        \begin{itemize}
   ...
   If $S$ is moreover strictly monotonic, we say the rule $\rho$ is
            \textbf{closure decreasing}\index{Closure decreasing} with respect to $\mathcal{T}$ in $\mathfrak{F}$ if the following conditions hold:
            \begin{itemize}
        ...
        \vspace{2mm}
            ...
            \begin{tikzpicture}
              \graphbox{\(L\)}{-50mm}{0mm}{40mm}{37mm}{2mm}{-10mm}{
                ...
                \node () at (-5mm,-17mm) {$\overset{h_{11}^2}{\to}$};
            \end{tikzpicture}
    \end{center}
