...
\begin{definition}[Decreasing Rules]
    ...
    The rule $\rho$ is \textbf{$\delta$-uniformly decreasing} w.r.t. $\mathcal{T}$ in $\mathfrak{F}$ if the following hold:
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \item[] $\bullet$ $\mu(w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K\}))  \mathop{>}   \mu(w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K\}))\mathop{+}\delta$.
            \end{itemize}
        \end{itemize}  
    ...
    The rule $\rho$ is
            ...
            \begin{itemize}
                ...
                \item[]- there exists a context closure $c_\rho$ for $\rho$ and $\mathcal{T}$ in $\mathfrak{F}$,
                \item[]- $ \mu(w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K\}))  \mathop{>}  \mu(w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K\})) \mathop{+}\delta$ for $t_K \mathop{=} l \mathop{\star} c_\rho$.
            \end{itemize}
\end{definition}

\begin{example}
    \label{example:decreasing_rule}
    Consider the DPO rule in Example~\ref{ex:grsaa} and the weighted type graph in~\autoref{example:weighted_type_graph} over the real arithmetic semiring $\mathfrak{N}' \mathop{=} (\mathbb{R}^+,+,*,0_\mathbb{R},1_\mathbb{R},<,\operatorname{id}_{\mathbb{R}^+})$. There are $t_K^{11}, t_K^{12}, t_K^{21}, t_K^{22}:K \mathop{\rightarrow} T$ as depicted below:
    ...
    \begin{center}
      ...
      \end{center}
    The set $\{l \mathop{\star} - \mathop{=} t_K^{11}\}$ has two morphisms $h_{11}^1$ and $h_{11}^2$ as illustrated below:
    \begin{center}
        \resizebox{0.49\textwidth}{!}{
        \begin{tikzpicture}
    ...
    Therefore, we have \begin{flalign*}
        ...
        \overset{\mathrm{def}}{=}&w_\mathcal{T}(h_{11}^1)\mathop{+}w_\mathcal{T}(h_{11}^2) \\
        =&(1.0^1 * 1.0^1)+(1.0^1 * 1.0^1)\\
        =&2.0
    \end{flalign*}
    The set $\{r \mathop{\star} - \mathop{=} t_K^{11}\}$ has one morphism $h_{11}^3$ as illustrated below:
    \begin{center}
        \resizebox{0.49\textwidth}{!}{
        \begin{tikzpicture}
      ...
      \end{center}
    Therefore, we have: $w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K^{11}\}) \mathop{=} w_\mathcal{T}(h_{11}^3) \mathop{=} 1.0^1 * 1.0^1 * 1.0 ^ 1 \mathop{=} 1.0$. Thus, we have $w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K^{11}\}) \mathop{=} 2.0 \mathop{\geq} 1.0 \mathop{=} w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K^{11}\})$.

    Similarly, we can check that $w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K^{12}\}) \mathop{=} 1.0 \mathop{\geq} 1.0 \mathop{=} w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K^{12}\})$,  $w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K^{21}\}) \mathop{=} 1.0 \mathop{\geq} 1.0 \mathop{=} w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K^{21}\})$, and $w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K^{22}\}) \mathop{=} 1.0 \mathop{\geq} 1.0 \mathop{=} w_\mathcal{T}(\{r \mathop{\star} - \mathop{=} t_K^{22}\})$. The rule is therefore weakly decreasing.

    There exists a context closure $c$ for the DPO rule in the weighted type graph, as shown in~\autoref{example:context_closure}.
    Since we have additionally $t_K^{11} \mathop{=} l \mathop{\star} c$ and $w_\mathcal{T}(\{l \mathop{\star} - \mathop{=} t_K^{11}\}) \mathop{=} 1.0 \mathop{>} 1.0\mathop{+}\delta$ for $\delta \mathop{=} 1.0$, the rule is $\delta$-closure decreasing with $\delta \mathop{=} 1.0$ since the semiring is strictly monotonic measurable.
...
\end{minipage}
  ...
  We have:
   \begin{itemize}
       \item $w_\mathcal{T}(G) \mathop{\succeq} w_\mathcal{T}(H)$ if $\rho$ is weakly decreasing, and
