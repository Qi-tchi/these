...
%     \end{tikzpicture} in the graph $T$,
    % and $w(e) = 1_\mathbb{N}$ for all $e \in \mathbb{E}$, is visualized in Figure~\ref{fig:weighted_type_graph_instance}. 
    To improve readability, this weighted type graph will visualized as shown in Figure~\ref{fig:weighted_type_graph_instance}, where a label $l$ with superscript $n$ on an edge means that there is a morphism-ruler with weight $n$ from \raisebox{2pt}{
