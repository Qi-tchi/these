...
\begin{example}
    Consider the weighted type graph $\mathcal{T}$ from Example~\ref{nwf:example:weighted_type_graph} and the morphisms shown in below:
    \begin{center}
    ...
    \begin{itemize}
        ...
        \begin{flalign*}
             0_S &= a \odot 0_S & \text{$0_S$ is annihilator for $\odot$}\\
                 ...
                 &\prec b \odot x \odot y &\text{by Axiom Equation~\eqref{ax:s4} and $x,y\neq 0_S$}\\
        ...
        Suppose
          ...
          If $1_S \prec x$ then $
        %   1_S \preceq y = 
          1_S \odot y \prec x \odot y$ by Axiom Equation~\eqref{ax:s4} since $y \neq 0$ by assumption.
        \item \ref*{eq:prop_neq0_plus_neq0}:  
        By Axiom Equation~\eqref{ax:s4}, we have $a \odot x \prec b \odot x$ and $a\odot y \prec b \odot y $. 
        \begin{flalign*}
            a \odot \left(x  \oplus y \right) &= \left( a \odot x \right)  \oplus \big(a \odot y \big)  & \text{by distributivity}\\
            & \prec \left(b \odot x \right)   \oplus \left( b \odot y \right)  & \text{by Axiom Equation~\eqref{ax:s2}}\\
...
% If the left pushout square is weighable with $\mathcal{T}$ and the right pushout square is bounded above by $\mathcal{T}$ (concepts defined in Definition~\ref{def:weighable}), then, by \cite[Lemma 5.10]{endrullis2024generalized_lmcs}, we derive the following assertions:
% \begin{flalign*}
%     w_\mathcal{T}(G) 
