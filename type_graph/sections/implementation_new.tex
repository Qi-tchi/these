We present the implementation of our approach and of the one proposed by Endrullis and Overbeek~\cite{endrullis2024generalized_arxiv_v2} in our tool, called LyonParallel, which is described in Chapter~\ref{chap:lyonparallel}.

\paragraph{Searching Strategy.}
Let $\mathcal{R}$ be a DPO rewriting system. For a specific semiring $S$, a processor is spawned to search iteratively for a suitable weighted type graph over $S$.
The search begins with a weighted type graph containing $1$ node and increments the node count by $1$ until it reaches a maximum of 4 nodes.
...
\begin{enumerate}
    \item decide whether $s \overset{l}{\to} t$ exists for every pair of nodes \( s, t\) and label \( l\);
...
This procedure amounts to checking 
the satisfiability of an existential Presburger arithmetic, Peano arithmetic, linear real arithmetic, or non-linear real arithmetic formula depending on the semiring $S$ considered.
...
a variable $y_{u,v,l}$ of Z3's \textit{Real} sort or \textit{Int} sort (the solver's default theory for real numbers and integers), depending on the semiring $S$ considered,
...
The variable $y_{u,v,l}$ are constrained to be non-negative.
...
\end{itemize}  
The value of $v_h$ has value \textit{true} if the morphism $h$ exists in the result type graph (i.e. all edges in its image exist), and \textit{false} otherwise.
The variable $v_{h'}$ holds the weight of the morphism $h$, provided the morphism $h$ exists (i.e., $v_h$ has value \textit{true}).
The variable $v_c$ has value \textit{true} if the morphism $c$ exists in the result type graph and can serve as a context closure for the rule, and \textit{false} otherwise.
