...
\begin{example}
  ...
  \end{figure}
   and the weighted type graph in \autoref{nwf:example:weighted_type_graphd_grsaa_sdfsf} over the real arithmetic semiring $\mathfrak{N}' = (\mathbb{R}^+,+,*,0_\mathbb{R},1_\mathbb{R},<,\operatorname{id}_{\mathbb{R}^+})$ shown below
    ...
    \end{figure}
     There are $t_K^{11}, t_K^{12}, t_K^{21}, t_K^{22}:K \rightarrow T$ as shown in~\autoref{fig:nwf:example:decreasing_rule_morphisms_grsaa_sfsdjl}.
    ...
    \begin{figure}[!ht]
      ...
      \end{figure}
    The set $\{l \star - = t_K^{11}\}$ has two morphisms $h_{11}^1$ and $h_{11}^2$ as illustrated in~\autoref{fig:nwf:dfsdfjejrhti3erjwm}.
    \begin{figure}[!ht]
        \centering
        \resizebox{0.49\textwidth}{!}{
        \begin{tikzpicture}
