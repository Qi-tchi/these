...
Adapted from \cite[Theorem C.3]{endrullis2024generalized_arxiv_v2} by Endrullis and Overbeek, the lemma below relates decreasing rules to weight reduction under specific constraints. \todo{todo to do: ref 3.30 incorrect}
...
$.
    ...
    \begin{enumerate}[label=(\alph*)] 
        ...
        \begin{proof}
            ...
            \underset{\substack{t_C:C \rightarrow T \\
            t_K = h_{KC} \star t_C }}{\bigoplus} 
            w_\mathcal{T}(t_C - h_{KC}) \succeq 1_S$ if there is be a morphism such that $t_K = h_{KC} \star t_C$ by Definition~\ref{def:real_strongly_monotonic_semiring}~\eqref{ax:s1}.
        \end{proof}
    ...
    \begin{center}
      ...
      \end{center}
        ...
        \begin{flalign*}
            ...
            \\ &=   l \star (h_{LG}  \star t_G) & \text{by Equation~\eqref{eq_tl_hlg_tg}\text{ above}}
            \\ &= (l \star h_{LG}) \star t_G &\text{by associativity }
            \\ &= (h_{KC} \star h_{CG}) \star t_G & \text{by commutativity of $\square KLGC$}
  ...
  By Lemma~\ref{nwf:lem_4d13}, we have 
    ...
    \noindent We complete the proof by case analysis:
    \begin{enumerate}
        ...
        \item
            ...
            Since the semiring is a strictly monotonic museurable semiring,  by \autoref{def:real_strongly_monotonic_semiring}~\eqref{ax:s2'} and Definition~\ref{def:real_strongly_monotonic_semiring}~\eqref{ax:s5'}, it suffices to show that there is some $t_K : K \to T$ such that 
            \begin{align}
