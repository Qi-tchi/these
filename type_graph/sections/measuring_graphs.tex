We recall definitions from~\cite{endrullis2024generalized_icgt}.
...
Combining the measurements of a morphism provided by all morphism-rulers with the help of a weight function $w$ gives the weight of the morphism.
\begin{definition} 
    \label{def:weight_of_a_morphism_relative_to_a_type_graph}
        Let $\mathcal{T}=(T,\mathbb{E},\mathcal{S},w)$ be a finitary weighted type graph.
         The \textbf{weight of a morphism $h: G \mathop{\rightarrow} T$ relative to a type graph $\mathcal{T}$}\index{Morphism weight!type graph method} is defined as the semiring product of $w(e)^{m_e(h)}$ for all $e \mathop{\in} \mathbb{E}$:
...
\begin{definition}[Object weight]
    \label{def:weight_of_an_object_relative_to_a_type_graph}
       Let $\mathcal{T}=(T,\mathbb{E},\mathcal{S},w)$ be a finitary weighted type graph. The \textbf{weight of an object \( G \)}\index{Object weight!type graph method} is defined as the semiring sum of $w_\mathcal{T}(h)$, for all \( h \mathop{\in} \operatorname{Hom}(G,T) \):
