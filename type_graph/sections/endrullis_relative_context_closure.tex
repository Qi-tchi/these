...
\begin{example}
   ...
   Consider the DPO rule in Example~\ref{ex:grsaa}, the weighted type graph in~\autoref{example:weighted_type_graph}.
   and the morphism $c$ illustrated below. Since for every match $m : L \mathop{\to} G$, there exists a morphism $\alpha : G \mathop{\to} T$ such that $m \mathop{\star} \alpha \mathop{=} c$, the morphism $c$ is a context closure for the DPO rule in the type graph.
  \begin{center}
    \begin{tikzpicture}
