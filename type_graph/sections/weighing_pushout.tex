% Let \( \mathcal{R} \mathop{=} \mathcal{A} \mathop{\cup} \mathcal{B} \) be a set of DPO rewriting rules and let $\mathcal{T}=(T,\mathbb{E},\mathcal{S},w)$ be a weighted type graph over a well-founded commutative semiring such that for every graph $G$ subject to rewriting, we have $\operatorname{Hom}(G,T)\mathop{\neq} \emptyset$. By~\autoref{rem:wf:weight_of_object_geq_1}, for all objects \( G \) subject to rewriting, \(w_\mathcal{T}(G) \mathop{\succeq}_S 1_\mathcal{S} \).
% The rewriting relation \( \mathop{\Rightarrow}_{\mathcal{A},\mathfrak{F}} \) is terminating relative to $\mathop{\Rightarrow}_{\mathcal{B},\mathfrak{F}}$ if (i) for all \(G \mathop{\Rightarrow}_{\mathcal{A},\mathfrak{F}} H\), \( w_\mathcal{T}(G) \mathop{\succ}_S w_\mathcal{T}(H)\), and (ii) for all \(G \mathop{\Rightarrow}_{\mathcal{B},\mathfrak{F}} H\), \( w_\mathcal{T}(G) \mathop{\succeq}_S w_\mathcal{T}(H) \). However, directly verifying all rewriting steps is infeasible due to the potentially infinite number of rewriting steps.
...
\begin{definition} 
    ...
    The \textbf{measurement of a morphism \( h:G \mathop{\to} T \) relative to a morphism-ruler \( e: X \mathop{\to} T \), excluding morphisms in \( \Gamma' \)}\index{Measurement of morphism!type graph method}, denoted by $m_e(h-\Gamma)$, is defined as:
    \[ 
...
\begin{definition}
    ...
    \index{Weight of morphism!type graph method}
...
An object $X$ is said to be \textbf{traceable along $\Delta$}\index{Traceable along} if for every diagram $\delta$ in $\Delta$, as shown below:
% in Figure~\ref{fig:preliminaries:pushout_square_traceable_sdlkfjsladkj}, and $h:X \mathop{\to} D$,      
% \begin{figure}[H]
%     \centering
\begin{center}
     \resizebox{0.5\textwidth}{!}{
...
\end{center}
  one of the following conditions holds:
    ...
    \index{Strongly traceable}
\end{definition}
Intuitively, in the category \textbf{Graph}, a graph $X$ is strongly traceable along a pushout square, if whenever $X$ occurs in $D$, it occurs either in $B$ or in $C$, and if it occurs in both, then it occurs in $A$ as well.
...
\begin{example}
   ...
   } is traceable in neither pushout square.
    
    The graph \tikz[baseline=-0.5ex]{
        \node[draw,circle] (x) at (0,0) {};
        \node[draw,circle] (y) at (1,0) {};
        \draw[->] (x) -- (y) node[midway, above] {$a$};
...
%      It is $u$-strongly traceable along the left pushout square, because the only morphism $h:X \mathop{\to} G$ factorizable through $m$ and $l$ has as image the graph \begin{tikzpicture}
%         \node[draw, circle] (x) at (0,0) {z};
%         \node[draw, circle] (y) at (1,0) {y};
%         \draw[->]  (x) -- (y) node [midway,above] {$c$};
%      \end{tikzpicture} and there is a unique morphism $k:X \mathop{\to} A$ such that $h \mathop{=} k \mathop{\star} l \mathop{\star} m$.
...
\begin{definition}[\cite{endrullis2024generalized_arxiv_v2}]
    ...
    \begin{itemize} 
        \item 
            \textbf{monic for $S$}\index{Monic for a set}
            if $g \mathop{\star} f \mathop{=} h \mathop{\star} f$ implies $g \mathop{=} h$ for all $g, h \mathop{\in} S$;
        \item 
            \textbf{$X$-monic}\index{$X$-monic} if $f$ is monic for $\operatorname{Hom}(X, A)$;
        \item \textbf{$X$-monic outside of $u$}\index{$X$-monic outside of $u$}, if $f$ is monic for \( \operatorname{Hom}(X,A) \mathop{\setminus} \left ( \operatorname{Hom}(X,C) \mathop{\star} u \right ) \);
        \item  \textbf{$\Gamma$-monic}\index{$\Gamma$-monic} if $f$ is $X$-monic for every $X \mathop{\in} \Gamma$.
    \end{itemize}
\end{definition} 
...
\begin{definition}[\text{\cite[\textdef~4.9]{endrullis2024generalized_arxiv_v2}}]
    \label{def:weighable}
       ...
       $\delta$ is said to be
         \begin{enumerate}[label=(\alph*)]
        \item \textbf{weighable}\index{Weighable} with $\mathcal{T}$ if the following conditions hold:
            ...
            \end{enumerate}
        \item \textbf{bounded-above}\index{Bounded-above} by $\mathcal{T}$ if $dom(\mathbb{E})$ is traceable along $\delta$.
    \end{enumerate}
   
\end{definition}


The following lemma shows that, under certain constraints, exact weights of pushout object can be precisely computed or upper-bounded.
\begin{lemma}[\text{\cite[Lemma 4.13]{endrullis2024generalized_arxiv_v2}}]
    \label{lem_4d13}
