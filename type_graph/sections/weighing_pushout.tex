...
\begin{lemma}[\text{\cite[Lemma 4.13]{endrullis2024generalized_arxiv_v2}}]
    \label{lem_4d13}
    Let $\mathcal{T} = (T,\mathbb{E}, S, w)$ be a finitary weighted type graph. Consider the pushout square $\delta$ illustrated on the right. 
...
\end{lemma}
    ...
    \end{center}
    If the left pushout square is weighable with $\mathcal{T}$ and the right pushout square is bounded above by $\mathcal{T}$, by \cite{lem_4d13}, we obtain:
