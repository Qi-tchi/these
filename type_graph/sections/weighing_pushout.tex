...
An object $X$ is said to be \textbf{traceable along $\Delta$} if for every diagram $\delta$ in $\Delta$, shown in~\autoref{fig:preliminaries:pushout_square_traceable_sdlkfjsladkj}, and $h:X \to D$,       
 the following hold
...
\end{definition}
    ...
    Consider a DPO diagram shown in~\autoref{fig:nwf:dpo_diagram_sdfslafgjsl} that defines a rewriting step \( G \Rightarrow_{\rho,\mathfrak{F}} H \). 
    \begin{figure}[H]
        \centering
        \resizebox{0.4\textwidth}{!}{
        \begin{tikzpicture}
