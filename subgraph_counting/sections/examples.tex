...
\begin{example}
    ...
    Consider the rewriting rule $\rho$ in~\autoref{ex:grsaa_rx}, let $X$ be the graph \tikz[baseline=-0.5ex]{
        \node (x) at (0,0) {$\bullet$};
        \node (y) at (1,0) {$\bullet$ }; 
        \node (z) at (2,0) { $\bullet$};
...
\begin{example} 
    ...
    \end{center}
        ...
        Therefore, the rule is $X$-non-increasing under $h_{R'_1L}$ and $h_{R'_2L}$. This is because all conditions in~\autoref{def:creates_more_x_on_the_left} are satisfied.
...
\begin{example} 
    ...
    The sets $D(R_\rho, X)$, $D(R_\rho, Y)$, $D(R_\tau, X)$ and $D(R_\tau, Y)$ are all empty. Therefore, both rules $\rho$ and $\tau$ are $X$- and $Y$-non-increasing as all conditions in~\autoref{def:creates_more_x_on_the_left} are met.
    ...
    Therefore, the system terminates by~\autoref{thm:termination_grs}.
...
\begin{center}
  ...
  \tikz[baseline=-0.5ex]{
      ...
      \node (y) at (1,0) {$\bullet$};
      \draw[->] (x) to (y);
  }. Both rules are $X$-non-increasing, and $|\operatorname{Mono}(X,L)| = 1 > 0 = |\operatorname{Mono}(X,R)|$ holds for both rules. Thus, this rewriting system terminates by~\autoref{thm:termination_grs}.
\end{example}
