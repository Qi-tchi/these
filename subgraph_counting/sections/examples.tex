...
\begin{example}[Limitation]
  ...
  \end{center}
    ...
    This happens because, for any system $\mathcal{R}$, rewriting step $G \Rightarrow_{\mathcal{R},\mathfrak{M}} H$, set of ruler-graphs $\mathbb{X}$, and weight function $s_\mathbb{X}$, we have $w_{s_\mathbb{X}}(G) \geq w_{s_\mathbb{X}}(H)$ because there is an injective graph homomorphism from $G$ to $H$.
  \end{example} 
