While the type graph method~\cite{bruggink2014termination, bruggink2015proving,endrullis2024generalized_arxiv_v2} can prove termination of the following example, it requires constructing an appropriate weighted type graph, a task that is difficult in general~\cite[\textsection 6]{bruggink2015proving}. 
...
\begin{example}
    ...
    the system terminates by Theorem~\ref{subgraph_counting:thm:termination_grs}.
\end{example}
The techniques from some previous works~\cite{bruggink2014termination,bruggink2015proving,endrullis2024generalized_arxiv_v2,plump2018modular,overbeek2024termination_lmcs} fail to prove termination of the following examples, whereas our method does.
\begin{example} 
    ...
    \begin{center}
        ...
        the function $\Psi$ that maps each element $R'_i \mathop{\in}  D(R,X)$ to $h_{R'_iL}$, because 
        the following diagrams can be constructed, and the remaining conditions in Definition~\ref{subgraph_counting:def:creates_more_x_on_the_left} are straightforward to verify. Since \(w_{s_\mathbb{X}}(L)\mathop{=}1\mathop{>}0\mathop{=}w_{s_\mathbb{X}}(R)\), the system terminates by Theorem~\ref{subgraph_counting:thm:termination_grs}.
        \begin{center}
...
The following example demonstrates the usefulness of a weight function that assigns distinct weights to measurements from different ruler-graphs.
\begin{example} 
...
\begin{example}[Limitation]
  ...
  Consider the rewriting rule~\cite[Example 5.6]{overbeek2024termination_lmcs}:
  \begin{center}
