...
\begin{notation}
 ...
 The disjoint union of two sets \( S \) and \( S' \) will be denoted by \( S \uplus S' \). Let \( X, A, B, G \) be graphs, and let \( \alpha \colon A \to G \) and \( \beta \colon B \to G \) be morphisms shown below:
\begin{center}
...
\end{flalign*}
\noindent By Lemma~\ref{subgraph_counting:lem:decomp_w_u}, the following equalities hold:
\begin{flalign*}
&\card{\operatorname{Mono}(X,G)} =
...
\begin{flalign*}
     ...
     \card{\operatorname{Mono}(X,H, \lnot r',\lnot m') } 
\end{flalign*}
By Lemma~\ref{subgraph_counting:lem:xlnlmxrnr}, it can be simplified to:
\begin{flalign*}
     \card{\operatorname{Mono}(X,G)} -  \card{\operatorname{Mono}(X,H)}
...
\begin{definition}[Distinguished subgraphs of the right-hand side graph]
    ...
    \begin{itemize}
        ...
        \item the following diagram where \(h_{K'K} \colon K' \rightarrowtail K \) and \(h_{R'R} \colon R' \rightarrowtail R \) are inclusion functions can be constructed:
        \begin{center}
            \resizebox{0.4\textwidth}{!}{
...
The third condition ensures that any $R' \in D(R,X)$ can be glued along some common interface elements with a subgraph $C'$ of a context graph \( C \) to form an \( X \)-occurrence. 
...
\begin{definition}[$X$-non-increasing rule]
    ...
    \begin{enumerate}
        \item For all $R' \in D(R,X)$, we can construct the following diagram where all morphisms other than $\Psi(R')$, $l$ and $r$ are inclusion functions:
      \begin{center}
        \resizebox{0.3\textwidth}{!}{
                \begin{tikzpicture} 
                    \node (k) at (0,0) {$K$}; 
...
\begin{example}
    ...
    % \end{figure}
    \end{center}
    Whenever \( R'_i \) (for \( 1 \leq i \leq 4 \)) forms an \( X \)-occurrence $x$ in $H$ that is included neither in \( R \) nor in $C$ with a subgraph $C' \subseteq C \cap x \subseteq C$, the graph
    \( \operatorname{\Psi(R')} \subseteq L\) forms a \( X \)-occurrence in $G$ with $C'$ that is included neither in \( L \) nor in $C$.
