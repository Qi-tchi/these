...
\begin{definition}
    ...
    \begin{enumerate}
        \item there are graphs $K'$, $C'$ and morphisms such that the following diagram where $h_{K'K}$ and $h_{R'R}$ are inclusion functions can be constructed:
        \begin{center}
            \resizebox{0.4\textwidth}{!}{
                \begin{tikzpicture}
                    \node (k) at (0, 0) {K};
...
T following lemma formalizes the intuition that if $\rho$ is $X$-non-increasing, then, for any rewriting step using $\rho$, 
