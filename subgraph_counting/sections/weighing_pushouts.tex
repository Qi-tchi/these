...
Counting monomorphisms from ruler-graphs before and after a rewriting step provides a termination criterion: 
    If there exists a ruler-graph $X$ such that every rewriting step using a rule in $\mathcal{A}$ strictly decreases the number monomorphism from $X$, and every rewriting step using a rule in $\mathcal{B}$ never increases it, then the rewriting relation induced by $\mathcal{A}$ is relatively terminating to the rewriting relation induced by $\mathcal{B}$.
...
\begin{example}
    ...
    Consider the injective DPO rule from~\cite[Example 6]{bruggink2014termination} shown in~\autoref{fig:subgraph_counting:grfksfdfgsdgsd}.  
    \begin{figure}[H]
        \centering
        \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
    ...
    }), because if it contains no interface node, Condition 1 of~\autoref{subgraph_counting:def:rx} would be violated due to $X$'s connectivity.
...
\begin{example}
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            \draw[->] (y) -- (z)   {};
    }. Consider the rule shown in~\autoref{fig:subgraph_counting:rx_counter_exdfs}.
    \begin{figure}[H]
...
\begin{example}
    ...
    in~\autoref{subgraph_counting:ex:grsaa_rx}. Let $X$ be the graph \tikz[baseline=-0.5ex]{
        \node[draw,circle] (x) at (0,0) { };
        \node[draw,circle] (y) at (1,0) { };
        \node[draw,circle] (z) at (2,0) { };
        \draw[->] (x) -- (y) node[midway, above] {$a$};
    ...
    The rule is $X$-non-increasing because the diagrams in~\autoref{fig:subgraph_counting:decodkfjasddgsf} can be constructed, and $h_{R'_1L}, h_{R'_2L}, h_{R'_3L}$ and $h_{R'_4L}$ satisfy all conditions in~\autoref{subgraph_counting:def:creates_more_x_on_the_left}. 
    %1
    \begin{figure}[H]
        \centering
        \resizebox{0.7\textwidth}{!}{
...
\begin{example} 
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            \draw[->] (y) -- (z)   {};
    }. The rewriting rule shown in~\autoref{fig:subgraph_counting:rx_counter_ex} is potentially $X$-increasing.
    \begin{figure}[H]
        \centering
        \resizebox{0.7\textwidth}{!}{
            \begin{tikzpicture}
        ...
        \caption{}
        \label{fig:subgraph_counting:rx_counter_ex}
    \end{figure}
...
\begin{example}
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            % \draw[->] (y) -- (z)   {};
    }. The rewriting rule shown in~\autoref{fig:subgraph_counting:degkdjfqaaacodkfjasddgsf} is potentially $X$-increasing.
    \begin{figure}[H]
        \centering
        \resizebox{0.7\textwidth}{!}{
            \begin{tikzpicture}
...
\begin{example}
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            \draw[->] (y) -- (z) {};
    }. The rewriting rule shown in~\autoref{fig:subgraph_counting:decodkfjasdddfsfdsagsf} is potentially $X$-increasing.
    \begin{figure}[H]
        \centering
        \resizebox{0.6\textwidth}{!}{
            \begin{tikzpicture}
