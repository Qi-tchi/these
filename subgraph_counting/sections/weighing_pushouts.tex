...
Define $\opn{Sub}(\opn{left}(\mathcal{R}))$ as the set of all subgraphs of left-hand sides graphs of rules in $\mathcal{R}$.
We call the elements of $\opn{Sub}(\opn{left}(\mathcal{R}))$~\textbf{ruler-graphs}. Like using rulers to measure physical objects, we use ruler-graphs (special reference structures) to measure graphs.
...
Counting monomorphisms from ruler-graphs before and after a rewriting step provides a termination criterion: 
    If there exists a ruler-graph $X$ such that every rewriting step using a rule in $\mathcal{A}$ strictly decreases the number monomorphism from $X$, and every rewriting step using a rule in $\mathcal{B}$ never increases it, then the rewriting relation induced by $\mathcal{A}$ terminates relative to the rewriting relation induced by $\mathcal{B}$.
...
However, systems like Example~\ref{subgraph_counting:ex:grsaa_rx} require more complex ruler-graphs such as \tikz[baseline=-0.5ex]{
    ...
    } as shown in Example~\ref{subgraph_counting:ex:termination:grsaa}.
...
\end{definition}
Assigning different weights to measurements from distinct ruler-graphs enhences the technique, as demonstrated in Example~\ref{ex:overbeek_5d6}.
\begin{definition} 
...
\end{flalign*}
\noindent Using Lemma~\ref{subgraph_counting:lem:decomp_w_u} 
(see \iflongversion
...
\begin{example}
    ...
    Consider the injective DPO rule from~\cite[Example 6]{bruggink2014termination} shown 
    below.
    ...
    Therefore, $R'$ must contain an interface node (either node 
    \tikz[baseline=-0.5ex]{ 
    ...
    Additionally, $R'$ must contain node 
    \tikz[baseline=-0.5ex]{ 
    ...
    Furthermore, $R'$ must not contain 
            \tikz[baseline=-0.5ex]{ 
    ...
    % \end{figure}
        \end{center}
    Thus, $D(R,X)$ consists of the four graphs $R'_1$, $R'_2$, $R'_3$ and $R'_4$ which are shown below.
    % in Figure~\ref{fig:subgraph_counting:decompdfjasdkjfgdsakjh}.
    For each graph, the construction of the pullback square in Condition 1 is demonstrated in Example~\ref{example:grs_aa:has_more_left} and the construction of the pushout square required by Condition 1 is straightforward. 
    % \begin{figure}[H]
    %     \centering
    \begin{center}
    ...
    \end{center}    
    %     \caption{}
    %     \label{fig:subgraph_counting:decompdfjasdkjfgdsakjh}
    % \end{figure}
\end{example} 
...
\begin{example}
    ...
    \tikz[baseline=-0.5ex]{ 
            ...
            \draw[->] (x) -- (y)   {};
            \draw[->] (y) -- (z)   {};
    }. Consider the rule shown below.
    % in Figure~\ref{fig:subgraph_counting:rx_counter_exdfs}.
    % \begin{figure}[H]
    \begin{center}
    ...
    %     \caption{}
    %     \label{fig:subgraph_counting:rx_counter_exdfs}
    % \end{figure}
    \end{center}
    Let $R' \in D(R,X)$. $R'$ must contain node 3, otherwise it would be a subgraph of $r(K)$, violating Condition 2 of Definition~\ref{subgraph_counting:def:rx}. Node 3 must be connected to an interface node, otherwise Condition 1 of Definition~\ref{subgraph_counting:def:rx} would be violated. Therefore, $R'$ must include \raisebox{2pt}{\scalebox{0.6}{\tikz[baseline=-0.5ex]{
...
\end{example}
We define the notion of \( X \)-non-increasing rules. Intuitively, if a rule $\rho$ is $X$-non-increasing, then for any rewriting step using $\rho$, there exists an injective mapping from the set of \( X \)-occurrences implicitly created by the step to the set of \( X \)-occurrences implicitly destroyed by the step. This will be proved in Lemma~\ref{subgraph_counting:lem:w_u_l_not_geq_r_not}.
...
% \end{definition} 
The first condition ensures that whenever $R' \in D(R,X)$ forms an $X$-occurrence in the result graph $H$, this occurrence has a corresponding $X$-occurrence relative to the morphism $h_{R'L}$ (as defined in Definition~\ref{def:x_occurrence}) in the host graph $G$.
The second condition guarantees that any $X$-occurrence implicitly created by the rewriting step has its corresponding $X$-occurrence not included in the context (Consequently, its corresponding $X$-occurrence is implicitly destroyed by the rewriting step). Counterexamples are provided in Example~\ref{subgraph_counting:ex:cond_2_necessaire} and Example~\ref{subgraph_counting:ex:cond_2_necessaire2}.
...
Counterexamples for these conditions are given in Example~\ref{subgraph_counting:ex:cond3_necessaire} and Example~\ref{subgraph_counting:ex:cond4_necessaire}.
...
\begin{example}
    ...
    The rule is $X$-non-increasing because the following diagrams
    %  in Figure~\ref{fig:subgraph_counting:decodkfjasddgsf} 
     can be constructed, and $h_{R'_1L}, h_{R'_2L}, h_{R'_3L}$ and $h_{R'_4L}$ satisfy all conditions in Definition~\ref{subgraph_counting:def:creates_more_x_on_the_left}. 
    %1
    % \begin{figure}[H]
    %     \centering
    \begin{center}
    ...
    %     \caption{}
    %     \label{fig:subgraph_counting:decodkfjasddgsf}
    % \end{figure}
    \end{center}
    ...
    consider the rewriting step defined by the DPO diagram 
    shown below.
    % in Figure~\ref{fig:subgraph_counting:decdfjaskldjf}. 
    % \begin{figure}[H]
    %     \centering 
    \begin{center}
        \resizebox{0.7\textwidth}{!}{
    ...
    %     \caption{}
    %     \label{fig:subgraph_counting:decdfjaskldjf}
    % \end{figure}
    \end{center}
    Whenever \( R'_i \) (for \( 1 \leq i \leq 4 \)) forms an \( X \)-occurrence $x$ in $H$ that is included neither in \( R \) nor $C$ with a subgraph $C' \subseteq C \cap x \subseteq C$, the graph
...
\begin{example}
    ...
    %     \caption{}
    %     \label{fig:subgraph_counting:degkdjfqaaacodkfjasddgsf}
    % \end{figure}
    \end{center}
...
\begin{example}
    ...
    %     \caption{}
    %     \label{fig:subgraph_counting:decodkfjasdddfsfdsagsf}
    % \end{figure}
    \end{center}
