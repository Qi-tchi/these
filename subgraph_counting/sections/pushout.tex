...
\begin{definition}[Diagram~\cite{barr1990category}]
    ...
    Let $\mathcal{C}$ be a category, and \( G \) an unlabeled graph. A \textbf{diagram} (of shape \( G \)) is a homomorphism of unlabeled graphs \( h : G \to \mathcal{C} \) where \( C \) is considered as an unlabeled graph. A diagram is said to be \textbf{commutative} if, for all nodes \( u \), \( v \), and any two paths from \( u \) to \( v \) in the unlabeled graph \( G \):
    ...
    where \([x]\) denotes the equivalence class of \(x\). Note that we have \([1_C]=[1_B]\) and \([2_C]=[2_B]\), and \(\{[1_B],[2_B],[3_C],[4_B]\}\) is isomorphic to \(\{1_D,2_D,3_D,4_D\}\).
     ...
     \begin{figure}[H]
      ...
      \begin{tikzpicture}
          ...
          \node () at (77mm,-8mm) {\( \overset{b}{\rightarrowtail} \)}; % K -> R
          \node () at (58mm,-18mm) {\( c\downarrowtail \)};
          \node () at (102mm,-18mm) {\( g\downarrowtail \)};
          \node () at (77mm,-33mm) {\( \overset{h}{\rightarrowtail} \)}; % C -> H
      \end{tikzpicture}
      }
      \caption{}
...
\begin{proposition}{\cite{corradini1997algebraic}[p.188]}
    In category \textbf{Graph} the pushout of two arrows always exists as well: It can be computed componentwise (as a pushout in \textbf{Set}) for the nodes and for the edges, and the source, target, and labeling mappings are uniquely determined.
\end{proposition}
