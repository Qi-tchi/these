 The concepts and notation in this section follow the treatments in~\cite{pierce1991basic,barr1990category}. The concept of category is introduced in~\autoref{def:cat}. Throughout this section, we fix a category \( \mathcal{C} \).
...
\begin{example} 
    Finite directed edge-labeled multigraphs and their homomorphisms form a category, hereafter denoted \textbf{Graph}. Its objects are labeled graphs, its morphisms are graph homomorphisms, and the monomorphisms are injective homomorphisms. 
    The set of homomorphisms from a graph $X$ to another graph $Y$ is denoted $\opn{Hom}(X,Y)$, and the set of monomorphisms from a graph $X$ to another graph $Y$ is denoted $\opn{Mono}(X,Y)$.
\end{example}
...
\begin{example}
Consider the diagram in the category \textbf{Graph} shown in~\autoref{fig:preliminaries:span_cospan}. \(
L \overset{\alpha}{\leftarrow} K \overset{\beta}{\rightarrow} C
\) is a span and \(L \overset{\beta'}{\rightarrow} G \overset{\alpha'}{\leftarrow} C \) is a cospan.
\end{example}
...
\begin{example}
    A commutative diagram in the category \textbf{Graph} of directed edge-labeled multigraphs is illustrated in~\autoref{fig:preliminaries:commutative_diagram}. The symbol $\mathop{=}$ in the center of the diagram
...
\begin{example}
    The diagram in~\autoref{fig:preliminaries:pushout_injective} is a pushout square in the category \textbf{Graph} of directed edge-labeled multigraphs. In this example, both $\alpha$ and $\beta$ are injective morphisms. Therefore, the pushout object $G$ can be constructed easily by taking the interface graph $K$ and adding elements from $L$ and $C$ which are not present in $K$.
    \begin{figure}[!ht]
        \centering
        \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture} 
...
\begin{example}
    The diagram in~\autoref{fig:preliminaries:pushout_non_injective} is a pushout square in the category \textbf{Graph} of directed edge-labeled multigraphs. In this example, $\beta$ is not injective. Therefore, some elements are merged in the pushout object $G$.
    ...
    \end{figure} 
\end{example}
The pullback is the dual construction of the pushout in category theory, and it can be thought of as construction of the interface structure along which two structures are glued together. (to do: double check)
...
\begin{example}
    The diagram in~\autoref{fig:preliminaries:pullback} is a pullback square in the category \textbf{Graph} of directed edge-labeled multigraphs. 
    \begin{figure}[!ht]
        \centering
        \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture} 
    ...
    \end{figure} 
    There are three other possible spans \( B \overset{\gamma'}{\leftarrow} E \overset{\gamma}{\rightarrow} C \), shown in~\autoref{fig:preliminaries:three_squares}, such that \(\gamma' \star \beta' = \gamma \star \alpha'\) holds, and for each of them, there exists a unique morphism \(\delta: E \to A\) such that $\gamma' = \delta \star \alpha$ and $\gamma = \delta \star \beta$, as shown in the following three diagrams:
    \begin{figure}[!ht]
        \centering
            \resizebox{0.7\textwidth}{!}{
            \begin{tikzpicture} 
