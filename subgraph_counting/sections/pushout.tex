...
\begin{example} 
    ...
    the pullback object $A$ is the set $A \mathop{=} \set{(b,c) \mid \beta (c) \mathop{=} \alpha (b)} \mathop{\subseteq} B \mathop{\times} C$; $\alpha$ and $\beta$ are defined as the corresponding projections, e.g., $\alpha((b, c)) \mathop{=} b$ and $\beta((b, c)) \mathop{=} c$.
