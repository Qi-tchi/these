...
\begin{example}
    The following unlabeled graph is a category with two nodes and four arrows with loops at each node as the identities and the recheability as the composition:
           \begin{center}
        \begin{tikzpicture}
...
\). An example of a span $(\alpha, \beta)$ is shown the following diagram.
...
\). An example of a cospan $(\beta', \alpha')$ is shown in the following diagram.
...
\begin{example}
    The following diagram is a commutative diagram in the category \textbf{Graph} of edge-labeled directed multigraphs. A symbol $\mathop{=}$ in the center of the diagram
