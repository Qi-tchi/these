...
\begin{proof}
    ...
    We are going to show that for two arbitrary distinct monomorphisms $h_{XH}', h_{XH}'' \mathop{\in} \operatorname{Mono}(X, H, \lnot m', \lnot r')$, there are two distinct monomorphisms $h_{XG}', h_{XG}'' \mathop{\in} \operatorname{Mono}(X, G, \lnot m, \lnot l')$.
    ...
    Analoguously, there exists a unique monomorphism $h_{XG}'':X \rightarrowtail G$ such that $h_{C''X} \mathop{\star} h_{XG} ''= h_{C''C} \mathop{\star} h_{CG}$ and $h_{R''X} \mathop{\star} h_{XG}'' \mathop{=} h_{R''L} \mathop{\star} h_{LG}$.
...
\begin{proof}
    ...
    \) holds for all rules $\rho \mathop{\in} \mathcal{A} \mathop{\cup} \mathcal{B}$ and for all rewriting steps $G \mathop{\Rightarrow}_{\rho, \mathcal{M}} H$.
