...
% %     \begin{itemize}
% %         \item The graph $T$ consists of a single node $v$, along with the set $\{(v, s, v) \mid s \mathop{\in} \Sigma\}$ as its set of arrows,
...
%     % $\Sigma$-labelled 
%     graph with at least one arrow. Define the weighted type graph $\mathcal{T}^X_\Sigma$ as the graph consists of a single node $v$, along with the set $\{(v, s, v) \mid s \mathop{\in} \Sigma\}$ as its set of arrows.
%     % ,
%     %     \item The unique morphism $e: X \mathop{\to} T$ is assigned the weight $1 \mathop{\in} \mathbb{N}$.
%     % \end{itemize}
% \end{definition}
...
%         \(
%         \set{ \alpha \mathop{\star} - \mathop{=} \gamma } \overset{def}{=} \set{ \beta \mathop{\in} \operatorname{Hom}(B,C) \mid \alpha \mathop{\star} \beta \mathop{=} \gamma} 
%         \); 
%         \item        
%             % For $\alpha : B \mathop{\to} C$ and $x : A \mathop{\to} C$, we define 
%             \(
%             \set{ - \mathop{\star} \beta \mathop{=} \gamma } \overset{def}{=} \set{ \alpha \mathop{\in} \operatorname{Hom}(A,B) \mid \alpha \mathop{\star} \beta \mathop{=} \gamma } 
%             \)
%             ; 
%         \item
...
%             \(
%                 S \mathop{\star} \beta \overset{def}{=} \set{ \alpha \mathop{\star} \beta \mid \alpha \mathop{\in} S }
%             \).
%     \end{itemize}
% \end{remark}
...
%         \item For $\Gamma \mathop{\subseteq} \homset{A}{G}$, the \textbf{weight of $\phi:G \mathop{\to} T$ excluding morphisms in $\Gamma' \overset{def}{=}\set{\iota \mathop{\in} \operatorname{Hom}(X,G) \mid \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta \mathop{\star} \alpha=\iota}$}, is defined as 
%         \[
%         w_{\mathcal{T}^X_\Sigma}(\phi - \Gamma) \overset{\operatorname{def}}{=} 
%         \left|\operatorname{Hom}(X,G) \mathop{\setminus} \Gamma' \right|
%         \]
...
%     %     \item The \textbf{weight of $\phi:G \mathop{\to} T$ excluding $\set{\iota \mathop{\in} \set{- * \phi \mathop{=} e} \mid \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta * \alpha=\iota}$}, for $\Gamma \mathop{\subseteq} \homset{A}{G}$, is 
%     %     \[
%     %     w_{\mathcal{T}^X_\Sigma}(\phi - \Gamma) \overset{\operatorname{def}}{=} 
%     %     \left|\set{
%     %             \iota \mathop{\in} \set{-*\phi=e} \mid \not \exists \alpha \mathop{\in} \Gamma. \exists \zeta.~ \zeta * \alpha \mathop{=} \iota 
%     %         }\right|
%     %     \]  
%     %     % Let \(\Gamma\) be a set of morphisms to \(G\). Let \(\Gamma'\) be the set \(\{ k \mathop{\in} \operatorname{Hom}(X, G) \mid \exists g \mathop{\in} \Gamma,  \exists f, f \mathop{\star} g \mathop{=} k\} \) of morphisms in \( \homset{X}{G} \) factorisable through some morphisms in \(\Gamma\). The \textbf{weight of the morphism \(h : G \mathop{\to} T\) excluding \( \Gamma' \)} relative to \(\mathcal{T}^X_\Sigma\) is defined as
%     %     % \[
