...
% %         \item a weight function  $w : \mathbb{E} \mathop{\rightarrow} \mathbb{N}$
% %     \end{itemize} 
% % \end{definition} 

% % \begin{definition}[Weighted Type Graph  \text{\cite{endrullis2023generalized}}]
% %     \label{def:t_sigm_x}
% %     Let $X$ be a $\Sigma$-labelled connected graph with at least one arrow. Define the weighted type graph $\mathcal{T}^X_\Sigma \mathop{=} (T, \{e\}, w)$ as follows:
% %     \begin{itemize}
% %         \item The graph $T$ consists of a single node $v$, along with the set $\{(v, s, v) \mathop{\mid} s \mathop{\in} \Sigma\}$ as its set of arrows,
% %         \item The unique morphism $e: X \mathop{\to} T$ is assigned the weight $1 \mathop{\in} \mathbb{N}$.
% %     \end{itemize} 
% % \end{definition}
...
%     % $\Sigma$-labelled 
%     graph with at least one arrow. Define the weighted type graph $\mathcal{T}^X_\Sigma$ as the graph consists of a single node $v$, along with the set $\{(v, s, v) \mathop{\mid} s \mathop{\in} \Sigma\}$ as its set of arrows.
%     % ,
%     %     \item The unique morphism $e: X \mathop{\to} T$ is assigned the weight $1 \mathop{\in} \mathbb{N}$.
%     % \end{itemize}
% \end{definition}
...
%     \begin{itemize}
%         \item
%         % For $\alpha : A \mathop{\to} B$ and $\gamma : A \mathop{\to} C$, we define 
%         \(
%         \set{ \alpha \mathop{\star} - \mathop{=} \gamma } \overset{def}{=} \set{ \beta \mathop{\in} \operatorname{Hom}(B,C) \mathop{\mid} \alpha \mathop{\star} \beta \mathop{=} \gamma} 
%         \); 
%         \item        
%             % For $\alpha : B \mathop{\to} C$ and $x : A \mathop{\to} C$, we define 
%             \(
%             \set{ - \mathop{\star} \beta \mathop{=} \gamma } \overset{def}{=} \set{ \alpha \mathop{\in} \operatorname{Hom}(A,B) \mathop{\mid} \alpha \mathop{\star} \beta \mathop{=} \gamma } 
%             \)
%             ; 
%         \item
%             % For $\alpha : B \mathop{\to} C$ and a
%             % set
%             For all $S \mathop{\subseteq} \operatorname{Hom}(A,B)$, we define
%             \(
%                 S \mathop{\star} \beta \overset{def}{=} \set{ \alpha \mathop{\star} \beta \mathop{\mid} \alpha \mathop{\in} S }
%             \).
%     \end{itemize}
% \end{remark}
% % The following definitions are adapted from \cite{endrullis2023generalized}. Intuitively, given a weighted type graph $\mathcal{T}^X_\Sigma$, a graph $G$ will be interpreted as the unique homomorphism $\phi : G \mathop{\to} T$, which will be interpreted as the number of graph homomorphisms from $X$ to $G$. The fundamental difference between the type graph method and many other existing termination methods is that instead of counting the number of graph occurrences, the type graph method counts the number graph homomorphisms which contain more informations. 
...
% %         \]
% %         \item For $\Gamma \mathop{\subseteq} \homset{A}{G}$, the \textbf{weight of $\phi:G \mathop{\to} T$ excluding morphisms in $\Gamma' \overset{def}{=}\set{\iota \mathop{\in} \operatorname{Hom}(X,G) \mathop{\mid} \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta * \alpha=\iota}$}, is defined as the number of morphisms from \( X \) to \( G \) that are not contained in $\Gamma'$:
% %         \[
% %         w_{\mathcal{T}^X_\Sigma}(\phi - \Gamma) \overset{\operatorname{def}}{=} 
% %         \left|\operatorname{Hom}(X,G) \mathop{\setminus} \Gamma' \right|
% %         \]  
% %         % Let \(\Gamma\) be a set of morphisms to \(G\). Let \(\Gamma'\) be the set \(\{ k \mathop{\in} \operatorname{Hom}(X, G) \mathop{\mid} \exists g \mathop{\in} \Gamma,  \exists f, f \mathop{\star} g \mathop{=} k\} \) of morphisms in \( \homset{X}{G} \) factorisable through some morphisms in \(\Gamma\). The \textbf{weight of the morphism \(h : G \mathop{\to} T\) excluding \( \Gamma' \)} relative to \(\mathcal{T}^X_\Sigma\) is defined as
% %         % \[
% %         % w_{\mathcal{T}^X_\Sigma}(h - \Gamma) \overset{\operatorname{def}}{=} \left|Hom(X,G) \mathop{\setminus} \Gamma'\right|
% %         % \]  
% %         \begin{center}
% %             \begin{tikzpicture}
% %                 \node (a) at (0,0) {X};
...
% The \textbf{weight of a morphism \( \phi : G \mathop{\rightarrow} T \)} relative to \(\mathcal{T}^X_\Sigma\) is defined as the number of morphisms from \( X \) to \( G \). The weight of a graph $G$ relative to $T_\Sigma$ is defined as the weight of the
% unique morphism from $G$ to $T_\Sigma$.

% For $\Gamma \mathop{\subseteq} \homset{A}{G}$, the \textbf{weight of $\phi:G \mathop{\to} T$ excluding morphisms in $\Gamma' \overset{def}{=}\set{\iota \mathop{\in} \operatorname{Hom}(X,G) \mathop{\mid} \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta \mathop{\star} \alpha=\iota}$}, is defined as the number of morphisms from \( X \) to \( G \) that are not contained in $\Gamma'$
% \begin{center}
%     \begin{tikzpicture}
%         \node (a) at (0,0) {X};
%         \node (b) at (2,1) {A};
...
%         \begin{itemize}
%             \item
%         The \textbf{weight of a morphism \( \phi : G \mathop{\rightarrow} T \)} relative to \(\mathcal{T}^X_\Sigma\) is defined as
%         \[
...
%         \item For $\Gamma \mathop{\subseteq} \homset{A}{G}$, the \textbf{weight of $\phi:G \mathop{\to} T$ excluding morphisms in $\Gamma' \overset{def}{=}\set{\iota \mathop{\in} \operatorname{Hom}(X,G) \mathop{\mid} \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta \mathop{\star} \alpha=\iota}$}, is defined as 
%         \[
%         w_{\mathcal{T}^X_\Sigma}(\phi - \Gamma) \overset{\operatorname{def}}{=} 
%         \left|\operatorname{Hom}(X,G) \mathop{\setminus} \Gamma' \right|
%         \]
...
%     %     \item The \textbf{weight of $\phi:G \mathop{\to} T$ excluding $\set{\iota \mathop{\in} \set{- * \phi \mathop{=} e} \mathop{\mid} \exists \alpha \mathop{\in} \Gamma. \exists \zeta.  ~\zeta * \alpha=\iota}$}, for $\Gamma \mathop{\subseteq} \homset{A}{G}$, is 
%     %     \[
%     %     w_{\mathcal{T}^X_\Sigma}(\phi - \Gamma) \overset{\operatorname{def}}{=} 
%     %     \left|\set{
%     %             \iota \mathop{\in} \set{-*\phi=e} \mathop{\mid} \not \exists \alpha \mathop{\in} \Gamma. \exists \zeta.~ \zeta * \alpha \mathop{=} \iota 
%     %         }\right|
%     %     \]  
%     %     % Let \(\Gamma\) be a set of morphisms to \(G\). Let \(\Gamma'\) be the set \(\{ k \mathop{\in} \operatorname{Hom}(X, G) \mathop{\mid} \exists g \mathop{\in} \Gamma,  \exists f, f \mathop{\star} g \mathop{=} k\} \) of morphisms in \( \homset{X}{G} \) factorisable through some morphisms in \(\Gamma\). The \textbf{weight of the morphism \(h : G \mathop{\to} T\) excluding \( \Gamma' \)} relative to \(\mathcal{T}^X_\Sigma\) is defined as
%     %     % \[
...
% This method is essentially an interpretation method \cite{nipkow1998term} for proving termination. It operates by first fixing a finite graph \(T\), called the \emph{type graph}, and an ordered set \((S, \mathop{\geq})\) which serve as a basis for associating weights to graphs. The process unfolds as follows:
% \begin{itemize}
%     \item Interpretation as Morphisms: A graph \( G \) is interpreted as a finite set of morphisms from \( G \) to \( T \).
%     \item Weight Association: Each morphism \( f: G \mathop{\rightarrow} T \) is assigned a weight from \(S\). This transforms the finite set of morphisms into a finite set of elements in \( S \).
%     \item Aggregation: An aggregator $\alpha : \mathcal{P}(S) \mathop{\to} S$ combines the finite set of weights into a single element in \( S \).
% \end{itemize}
...
%     \item \textbf{Weight Assignment}: Each morphism \( f: G \mathop{\rightarrow} T \) is assigned a weight from \( S \), converting the set of morphisms into weights.
%     \item \textbf{Aggregation}: An aggregator function \( \alpha : \mathcal{P}(S) \mathop{\to} S \) combines the set of weights into a single element in \( S \).
% \end{itemize}
...
\begin{definition}[Flower Type Graph]
    ...
    % A \textbf{flower type graph} \( \mathcal{T}_\Sigma^X \mathop{=} (\mathcal{T}, e : X \mathop{\to} T) \) consists of 
    % \begin{itemize}
