...
In the one hand,
 their method is sensible to the interface choice as mentioned in \cite[Example 5.5]{Overbeek2024subgraph}. For example, it can prove the termination of \cite[Example 5.5]{Overbeek2024subgraph} but not the termination of \cite[Example 6.3]{EndrullisOverbeek2024Generalized}, while the these two rewriting rules are exactly the same except that the latter has an discrete interface. The same for Example~\ref{ex:termination:contrib} and~\autoref{ex_contrib_variant}.
\begin{example} 
