...
The left-hand sides of rules are typical examples
of such patterns, and we leave it to future work to explore whether other patterns are useful.

Let \( \mathbb{X} \) denote a fixed set of patterns. We assign weights (natural numbers) to monomorphisms 
$x : X \to G$ with $X \in \mathbb{X}$, and define the weight of a graph $G$ as the sum of the weights of all monomorphisms from graphs in \( \mathbb{X} \) to $G$. 
...
\end{itemize}
    We suppose additionally that there is an injective mapping from the set of monomorphisms whose image is neither fully included in $m'(R)$ nor fully included in $r'(C)$ to the set of monomorphisms whose image is neither fully included in $m(L)$ nor fully included in $l'(C)$. 
...
Using this definition, we outline the general approach and identify the key challenge in~\textsection~\ref{subgraph_counting:sec:general_idea} and we introduce the concept of non-increasing rules in \textsection~\ref{subgraph_counting:sec:non-increasing}. 
