The morphism-counting method presented in this chapter is specifically designed for DPO rewriting systems with injective rules and matches. However, we employ an equivalent definition of these systems due to its practical advantage: it provides a clearer perspective on how the number of monomorphisms from specific graphs changes following a rewriting step.
...
\begin{definition}
    ...
    % \end{itemize}
Let \(\Sigma\) be a finite set of labels. A \textbf{directed edge-labeled multigraph}\index{Directed edge-labeled multigraph} is a structure \((G, V_{\nexists})\) where
    \begin{itemize}
        \item G is a graph,
...
A homomorphism of pregraphs is a homomorphism between their underlying graphs that preserves missing.
%  \begin{definition} 
...
Let $C$ and $R$ be subpregraphs of a pregraph $G$. Their \textbf{union}, denoted $C \cup R$, is the pregraph whose underlying graph is the union of the underlying graphs of $C$ and $R$, and whose set of existing nodes is:
