The morphism counting method presented in this chapter is specifically designed for DPO rewriting systems with injective rules and matches. However, we employ an equivalent definition of these systems because it facilitates the presentation of our results.
...
Graphs can be decomposed into unions of pregraphs. These pregraphs are used to model finite directed edge-labeled multigraphs that may contain dangling edges.
\begin{definition}
    \label{def:pregraph}\index{Pregraph}
    % A \textbf{pregraph} $G$ consists of
    % \begin{itemize}
    ...
    % \end{itemize}
Let \(\Sigma\) be a finite set of labels. A \textbf{pregraph} is a structure \((G, V_{\nexists})\) where
    \begin{itemize}
        \item G is a graph,
        \item $V_{\nexists}$ is a subset of nodes of $G$ called \textbf{missing nodes}.
    \end{itemize}
\end{definition} 
For a pregraph \( G \), we write \( V_{\nexists}(G) \) for its set of missing nodes, and denote the set of existing nodes by $V_\exists(G)= V(G) \mathop{\setminus} V_{\nexists}(G)$.
...
A homomorphism of pregraphs is a homomorphism between their underlying graphs that preserves missing nodes.
%  \begin{definition} 
...
% \end{definition}
\begin{definition}
    \label{def:subpregraph}
    \index{Subpregraph}
    A pregraph $H$ is a \textbf{subgraph} of a pregraph $G$ (denoted $H \subseteq G$) if:
    \begin{enumerate}
        \item the underlying graph of $H$ is a subgraph of the underlying graph of $G$,
        \item the status of a node (whether it is existing or missing) is inherited from $G$:
        \[ V_\exists(H) = V(H) \cap V_\exists(G) \quad \text{and} \quad V_{\nexists}(H) = V(H) \cap V_{\nexists}(G) \]
    \end{enumerate}
\end{definition}

\index{Union of pregraphs}
Let $C$ and $R$ be subpregraphs of a pregraph $G$. Their \textbf{union}, denoted $C \cup R$, is the pregraph whose underlying graph is the union of the underlying graphs of $C$ and $R$, and whose set of missing nodes is:
\[ V_{\nexists}(C \cup R) = (V_{\nexists}(C) \cup V_{\nexists}(R)) \setminus (V_\exists(C) \cup V_\exists(R)) \]
...
Graphs in a DPO rewriting rule can be decomposed into unions of pregraphs as shown below, where different colors are used to distinguish the pregraphs.
   \begin{center}
   ...
   \end{center}
This decomposition is abstractly represented as follows:
...
\begin{center}
   ...
   Graphs in a DPO rewriting step can likewise be decomposed into unions of pregraphs as shown below:
...
\begin{figure}[H]
    ...
    \resizebox{0.9\textwidth}{!}{
          ...
          \caption{Decomposition of graphs in a DPO rewriting step}
          \label{fig:dpo_decomposition_pregraph}
   \end{figure}
...
% Let $R$ be a subpregraph of a pregraph $H$. The \textbf{relative complement} of $R$ in $H$, denoted $H \setminus R$, is the pregraph whose underlying graph is the subgraph of the underlying graph of $H$ formed by the edge set $X \isdef E(H) \setminus E(R)$ and the set $Y$ of their incident nodes, and whose set of missing nodes is:
% \[ V_{\nexists}(H \setminus R) = \left( V_{\nexists}(H) \cup (V_\exists(H) \cap V_\exists(R)) \right) \cap Y \]

% The \textbf{relative complement} of $R$ in $H$ where $R \mathop{\subseteq} H$, denoted by $H \mathop{\setminus} R$, is the pregraph with
% \begin{itemize}
...
The decomposition shown in~Figure~\ref{fig:dpo_decomposition_pregraph} provides a convenient way to classify morphisms into the host and result graphs of a rewriting step.
\begin{definition}
    ...
    Given an implicit $X$-occurrence $x$ in $H$, its image can be decomposed as the union $R_x \mathop{\cup} C_x$ of pregraphs, where $R_x \mathop{=} \opn{Im}(x) \mathop{\cap} R$ (the part of the image overlapping the right-hand side $R$), and $C_x \mathop{=} \opn{Im}(x) \mathop{\cap} C'$ (the part overlapping the context $C$).
    ...
    For every graph monomorphism $h : R_x \rightarrowtail L$ that preserves interface elements (i.e., \( h(k) \mathop{=} k \) for all \( k \mathop{\in} K \)), we define the \textbf{corresponding $X$-occurrence of $x$ relative to $h$} as the $X$-occurrence $y$ in $G$ given by:
        ...
        y(z) \mathop{=} 
        \begin{cases}
        x(z) & \text{if } z \mathop{\in} x^{-1}(C_x), \\
        h(x(z)) & \text{if } z \mathop{\in} x^{-1}(R_x).
        \end{cases}
        $$
   \end{definition} 
...
\begin{itemize}
    ...
    \item 1 implicit occurrence in $G$:
    \tikz[baseline=-0.5ex]{ 
...
\end{itemize}
    The corresponding X-occurrence of the implicit occurrence \tikz[baseline=-0.5ex]{ 
        \node[red,draw,circle] (x) at (0,0) {5};  
        \node[draw,circle] (y) at (1,0) {2};
        \node[blue,draw,circle] (z) at (2,0) {6};
        \draw[red,->] (x) -- node[midway,above] {$a$} (y) ;
