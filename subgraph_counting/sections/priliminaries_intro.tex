...
\begin{definition}
    ...
    An \textbf{inclusion}\index{inclusion function} is a function $f$ such that $\opn{dom}(f) \mathop{\subseteq} \opn{codom}(f)$ and $f(x) \mathop{=} x$ for every $x \mathop{\in} \opn{dom}(f)$.
    ...
    % the commutative diagram illustrated in Figure~\ref{fig:subgraph_counting:general_rewriting_step_sdfsl_sdfjlsdkjfl} can be constructed:
            \begin{center}
                        \begin{tikzpicture}
                                \node (I) at (0,0) {$K'$};
                                \node (L) at (-2,0) {$L'$};
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item a source function, denoted by \( \textbf{src}_G \)\index{Source function \( \textbf{src}_G \)}, (resp. a target function, denoted by \( \textbf{dst}_G \)\index{Target function \( \textbf{dst}_G \)}) that assigns to each edge an existing or missing node,
...
The set of existing nodes, $V(G) \mathop{\setminus} V_{\nexists}(G)$, is denoted by $V_\exists(G)$.
 ...
 \begin{definition} 
    ...
    Let \( G \) and \( H \) be pregraphs. A \textbf{homomorphism of pregraphs}\index{Homomorphism!of pregraphs} $h: G \mathop{\to} H$ consists a pair of functions $h_V: V(G) \mathop{\to} V(H) $ and $h_E: E(G) \mathop{\to} E(H)$ such that
    \begin{itemize}
...
The \textbf{union}\index{Union} $C \mathop{\cup} R$ of two subpregraphs $C \mathop{\subseteq} G$ and $R \mathop{\subseteq} G$ is a pregraph with 
\begin{itemize}
...
\subsection{X-occurrences}
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    Since all arrows are inclusions, by Definition~\ref{def:rewriting_inclusion}, graphs can be decomposed as unions of pregraphs as illustrated below, 
...
\begin{definition}
    \label{def:x_occurrence}
     Let $X$ be a subgraph of $L$.  An \textbf{$X$-occurrence}\index{$X$-occurrence} in a graph $G$ is a monomorphism from $X$ to $G$. An $X$-occurrence in $G$ is 
    \begin{itemize}
