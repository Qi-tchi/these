...
\begin{center}
    ...
    %     }         
    % \end{center}
The \textbf{relative complement} of $R$ in $H$ where $R \subseteq H$, denoted by $H \setminus R$, is the pregraph with
\begin{itemize}
...
\subsection{$X$-occurrences}
    ...
    % \end{figure}
    \end{center}
    Since all arrows are inclusions, by Definition~\ref*{def:rewriting_inclusion}\todo{todo to do: ref incorrect}, graphs can be decomposed as unions of pregraphs as illustrated below, 
     where $K, L', R',C'$ are pregraphs and $L = L' \cup K$, $R = R' \cup K$, $C = C' \cup K$, $G = L' \cup K \cup C'$, $H = R' \cup K \cup C'$.
    % \begin{figure}[H]
    %     \centering
    \begin{center}
    \resizebox{0.49\textwidth}{!}{
