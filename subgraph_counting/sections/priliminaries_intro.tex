...
\begin{definition}
    ...
    Two rules $\varphi' \mathop{=} (L' \overset{l'}{\leftarrowtail} K' \overset{r'}{\rightarrowtail} R')$ and $\varphi$ are \textbf{equivalent} if there exist isomorphisms $a,b,c$ such that 
    ...
    %  in Figure~\ref{fig:subgraph_counting:gedfadsdsfdsfds}
...
% in Figure~\ref{fig:subgraph_counting:union_of_pregraphs}.
% \begin{figure}[H]
%     \centering
\begin{center}
...
% in Figure~\ref{fig:subgraph_counting:relative_complement_of_pregraphs}.
\begin{center}  
% \begin{figure}[H]
    %     \centering
  \resizebox{0.9\textwidth}{!}{
    ...
    % \caption{Relative complement of $R$ in $H$.}
...
\subsection{$X$-occurrences}
    ...
    % illustrated in Figure~\ref{fig:subgraph_counting:pushoutsquare} can be decomposed into the diagram illustrated in Figure~\ref{fig:subgraph_counting:decomposition_of_rewriting_step}.
    % \begin{figure}[H]
    %     \centering
    \begin{center}
