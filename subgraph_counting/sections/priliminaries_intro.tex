...
\begin{definition}
    ...
    % the commutative diagram illustrated in Figure~\ref{fig:subgraph_counting:general_rewriting_step_sdfsl_sdfjlsdkjfl} can be constructed:
            \begin{center}
                ...
                \end{center}
    A rewriting step $G \Rightarrow_\varphi H$ is a double-pushout as shown
    below:
...
\begin{center}
    ...
    %     }         
    % \end{center}
The \textbf{relative complement} of $R$ in $H$ where $R \subseteq H$, denoted by $H \setminus R$, is the pre-graph with
\begin{itemize}
...
\subsection{$X$-occurrences}
    ...
    % \end{figure}
    \end{center}
    Since all arrows are inclusions, by Definition~\ref{def:rewriting_inclusion}, graphs can be decomposed as unions of pre-graphs as illustrated below, 
     where $K, L', R',C'$ are pre-graphs and $L = L' \cup K$, $R = R' \cup K$, $C = C' \cup K$, $G = L' \cup K \cup C'$, $H = R' \cup K \cup C'$.
    % \begin{figure}[H]
    %     \centering
    \begin{center}
    \resizebox{0.49\textwidth}{!}{
...
Consider the $X$-occurrences in the decomposition of a rewriting step using the rule in Example~\ref{ex:grsaa} shown below:
...
%   \end{figure}
    \end{center}
    This decomposition is illustrated below
    % in Figure~\ref{fig:subgraph_counting:occurrendfsdadsgces_of_x} 
    using the visual notation for graph homomorphisms introduced in Notation~\ref{notation:graph_homomorphism}, with distinct pre-graphs highlighted in different colors.
% \begin{figure}[H]
%     \centering
    \begin{center}
