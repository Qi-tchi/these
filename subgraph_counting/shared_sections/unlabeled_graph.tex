...
%         \item a function $\opn{dom}: E \mathop{\to} V$ assigning to each edge its \textbf{source} (or \textbf{domain}) node,
%         \item a function $\opn{cod}: E \mathop{\to} V$ assigning to each edge its \textbf{target} (or \textbf{codomain}) node.
%     \end{itemize} 
...
%     We write \( a: s \mathop{\to} t \) to indicate that \( a \) is an edge from \( s \) to \( t \).
% \end{definition}   

% Note that an unlabeled graph can have multiple edges from one node to another, as well as loops (edges from a node to itself), because different edges $u,v$ of the unlabeled graph can have the same source and target nodes, i.e. $\opn{dom}(u) \mathop{=} \opn{dom}(v)$ and $\opn{cod}(u) \mathop{=} \opn{cod}(v)$.
