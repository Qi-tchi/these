We consider finite directed edge-labeled graphs that may contain parallel edges, as used in earlier work on termination of graph transformation systems \cite{bruggink2014termination,bruggink2015proving,endrullis2024generalized_icgt,overbeek2024termination_lmcs}.
 This notion differs from that of Barr and Wells \cite{barr1990category}, whose directed multigraphs with loops lack edge labels; from that of König et al. \cite{konig2018atutorial}, since we permit the collections of nodes and edges to be classes; and from that of Ehrig et al. \cite{ehrig1997handbook1}, where both nodes and edges are labeled, whereas here only edges are labeled.
...
\begin{definition}
    \label{def:graph}
     Let \(\Sigma\) be a finite set of labels. A \textbf{directed edge-labeled multigraph}\index{Directed edge-labeled multigraph} is a structure \((V, E, \opn{dom}, \opn{cod},\lambda)\) where
    \begin{itemize}
        ...
        \item $\lambda: E \mathop{\to} \Sigma$ is the \textbf{labeling} function assigning to each edge a label.
    \end{itemize}
    A directed edge-labeled multigraph is said to be \textbf{finite}\index{Directed edge-labeled multigraph!finite} if $V$ and $E$ are finite, and \textbf{discrete}\index{Directed edge-labeled multigraph!discrete} if its edge set \(E\) is empty.
...
\end{definition}
A directed edge-labeled multigraph may have multiple edges from one node to another, as well as loops (edges from a node to itself), because different edges $u,v$ can have the same source and target nodes, i.e. $\opn{dom}(u) \mathop{=} \opn{dom}(v)$ and $\opn{cod}(u) \mathop{=} \opn{cod}(v)$. Nodes are not labeled, but node labeling can be simulated by adding self-loops.

    Hereafter, a directed edge-labeled multigraph will be simply referred to as a \enquote{\textbf{labeled graph}}\index{Labeled graph} or \enquote{\textbf{graph}}\index{Graph} when the context makes it clear, and we fix a set $\Sigma$ of labels.
       ...
       \resizebox{0.35\textwidth}{!}{
        \begin{tikzpicture}
            \graphbox{}{0mm}{0mm}{32mm}{28mm}{-10mm}{-14mm}{
                ...
                \draw[->] (1) edge[loop above,yshift=-2pt] node[midway, above] {$a$} (1) ;
                \draw[->] (1) edge[loop below,yshift=2pt] node[midway, below] {$b$} (1) ;
                \draw[->] (1) edge[bend left] node[midway, above,yshift=-2pt] {$a$}  (2)  ;
                \draw[->] (1) edge node[midway, above,yshift=-2pt] {$a$}  (2)  ;
                \draw[->] (2) edge[bend left] node[midway, below,yshift=2pt] {$a$} (1)   ;
            }
        \end{tikzpicture}
       }
