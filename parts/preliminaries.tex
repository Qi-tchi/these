    ...
    \end{definition}
      ...
      \begin{definition}[Rewriting system]
        ...
        \begin{itemize}
          ...
          \item $\mathfrak{M}$ is a function, called \textbf{match mechanism}, which associates to each $(r,d) \in \mathcal{R} \times D$ a set $\mathfrak{M}(r,d)$ of matches in $M$, called \textbf{matches of the rewriting rule $r$ in the object $d$}, 
           
          \item $\mathfrak{W}$ is a function, called \textbf{witness function}, which associates to each $(r,d,m) \in \mathcal{R} \times D \times M$ a witness of rewriting step $\mathfrak{W}(r,d,m)$ in $W$, called \textbf{witness of the rewriting step defined by the match $m$ of the rule $r$ in the object $d$},
        ...
        For every rule $\rho \in \mathcal{R}$, it induces a rewriting relation $\Rightarrow_\rho$ that is defined as follows: for all objects $s, t \in D$, the rewriting step $s \Rightarrow_\rho t$ exists
          iff there exists a match $m \in \mathfrak{M}(\rho,s)$ of the rule in the object $s$ such that $\mathfrak{W}(r,s,m)$ witnesses the rewriting step, i.e. $\mathfrak{I}(\mathfrak{W}(r,s,m)) = (s,t)$. 
          
          The \textbf{rewriting relation} $\Rightarrow_\mathcal{R}$ is defined as follows: for all objects $s, t \in D$, the rewriting step $s \Rightarrow_\mathcal{R} t$ exists iff $s \Rightarrow_\rho t$ for some rule $\rho \in \mathcal{R}$.

          For a rewriting step $s \Rightarrow_\rho t$ with match $m$, we denote $s \Rightarrow_\rho^m t$ to indicate the match $m$ used to rewrite $s$ to $t$.
      ...
      \begin{definition}[Rewriting system in a rewriting framework]
        ...
        \begin{itemize}
          ...
          \item $\mathfrak{F}$ is a mapping, called \textbf{rewriting framework}, which associates to each rule $r \in R$, 
             a collection of witnesses in $W$.
        \end{itemize}
        
        For every rule $\rho \in \mathcal{R}$, it induces a rewriting relation $\Rightarrow_{\mathfrak{F},\rho}$ that is defined as follows: for all objects $s, t \in D$, the rewriting step $s \Rightarrow_{\mathfrak{F},\rho} t$ exists
          iff $s \Rightarrow_\rho^m t$ for some match $m \in \mathfrak{M}(\rho,s)$ and $\mathfrak{W}(r,s,m) \in \mathfrak{F}(\rho)$.

          The \textbf{rewriting relation} $\Rightarrow_{\mathfrak{F},\mathcal{R}}$ is defined as follows: for all objects $s, t \in D$, the rewriting step $s \Rightarrow_{\mathfrak{F},\mathcal{R}} t$ exists iff $s \Rightarrow_{\mathfrak{F},\rho} t$ for some rule $\rho \in \mathcal{R}$.

          For a rewriting step $s \Rightarrow_{\mathfrak{F},\rho} t$ with match $m$, we denote $s \Rightarrow_{\mathfrak{F},\rho}^m t$ to indicate the match $m$ used to rewrite $s$ to $t$.
...
%   A rewriting system $\mathcal{R}$ is terminating if there is a terminating rewriting system $\mathcal{S}$ and functors $h_1,\ldots, h_n$ such that $h_1 \star \ldots \star h_n : \mathcal{R} \to \mathcal{S}$
% \end{corollary}
...
\chapter{Graph Relabeling Systems}
    ...
    \begin{notation}
        ...
        This is illustrated in the following representation of a homomorphism \( h: G \to H \).
...
\begin{definition}[GLS rewriting rule]
  \label{def:gls_rule}
  A \textbf{GLS rewriting rule} is an ordered pair of labeled graphs \( \rho = (U,\lambda) \to (U, \lambda') \) that share the same underlying unlabeled graph \( U \). The labeled graph \( (U, \lambda) \) is said to be the \textbf{left-hand side graph}, denoted $\opn{lhs}(\rho)$, and \( (U, \lambda')\) the \textbf{right-hand side graph}, denoted $\opn{rhs}(\rho)$.
\end{definition} 
...
\begin{definition}[Graph relabeling]
  ...
  Let \( \rho = (L, \lambda_L) \to (R,\lambda_R) \) be a GLS rewriting rule, \( (G, \lambda_G) \) a graph.

  A \textbf{GLS match} of \( \rho \) in \( G \) is an injective graph homomorphism \( m : (L, \lambda_L) \rightarrowtail (G, \lambda_G) \). 
  
  Let $\lambda'_G$ be the labeling function on nodes and edges of \( G \) such that \( \lambda'_G(x) = \lambda_G(x) \) for all nodes and edges \( x \) in \( G \setminus \operatorname{Im}(m) \), and \( \lambda'_G(x) = \lambda_R(m^{-1}(x)) \) otherwise.

  Let $m' : (R, \lambda_R) \rightarrowtail (G, \lambda'_G)$ be the injective graph homomorphism with \( m(x) = m'(x) \) for all nodes and edges \( x \) in \( G \).
...
% Let $\to_\mathcal{AC}$ be the rewriting relation defined by $\mathcal{AC} \cup \mathcal{C}$.
% \begin{definition}[ACTRS rule]
%    A \textbf{ACTRS rule} is a TRS rule.
% \end{definition}

% \begin{definition}[Match]
%     Let $[s] \in T(\Sigma, \mathcal{X})/\to_\mathcal{AC}$ and $\rho = l \to r$ be an ACTRS rule. 
%     An ACTRS match of $\rho$ in $[s]$ is an ordered pair $(s',p)$ where $s' \in [s]$ and $p \in \operatorname{Pos}(s')$ such that there exists a substitution $\sigma$ such that $s'_{|p} = \sigma(l)$.
% \end{definition}

% \begin{definition}[ACTRS Rewriting step]
%     Let $[s] \in T(\Sigma, \mathcal{X})/\to_\mathcal{AC}$ and $\rho = l \to r$ be an ACTRS rule. Let $(s',p)$ be a match of $\rho$ in $[s]$.

%     The structure $(s', p, \rho)$ is a \textbf{witness} for the \textbf{ACTRS rewriting step} $[s] \to_\mathcal{R} [t]$
%      using the rule $\rho$ and match $(s',p)$ where $t \in T(\Sigma, \mathcal{X})$ such that $s' \to_{trs}^p t$.
% \end{definition}
...
\begin{definition}[Rewriting of terms with associative and commutative symbols~\cite{urbain2001approche}]
  ...
  % The \textbf{ACTRS witness function} is the function $\mathfrak{W}$ that associates to each ACTRS match $(\rho, s, p, \sigma)$ the rewriting step $(s, t)$ where $t = s[\sigma(r)]_p$.
\end{definition}

% \begin{definition}[Term Rewriting modulo AC framework]
%   Let $\mathcal{R} \subseteq T(\Sigma,\mathcal{X})^2$ be a set of term rewriting rules.
   
%   For all $\rho \in \mathcal{R}$, we define the the rewriting framework $\f{F}_{actrs}(\rho)$ as follows:
%   \begin{flalign*}
...
%         \right \}
%   \end{flalign*}
%   An element $(s,t) \in \f{F}_{actrs}(\rho)$ is called a \textbf{witness} for the rewriting step $[s] \to_\mathcal{R} [t]$ using the rule $\rho$ where $[s],[t] \in T(\Sigma,\mathcal{X})/\to_\mathcal{AC}$. 
...
%     The rewriting relation $\to_\mathcal{R}^!$ is defined as follows: $s \to_\mathcal{R}^! t$ iff there exists a finite sequence of rewriting steps $s \to_\mathcal{R}^n t$ for some $n \in \mathbb{N}$ and $t \in T(\Sigma, \mathcal{X})$ is in $\to_\mathcal{R}$-normal form.
% \end{definition}
...
%          For every $\rho \in \mathcal{R}$, 
%         %  let $(T(\Sigma,\mathcal{X}), \to_{\rho/AC})$ be the term rewriting system modulo AC induced by $\{\rho\} \subseteq \mathcal{R}$ in the framework $\f{F}_{actrs}$, we define
%         % \begin{flalign*}
...
%           \f{F}_{nactrs}(\rho) \isdef 
%             \left \{(s,t)\mid 
%                 \exists s'.~s \to_\mathcal{S}^!~s'
%                 \to_\mathcal{AC}^* s'' \to_\rho t' \to_\mathcal{AC}^* t
%                 % \exists C. \exists p \in Pos(C). \exists \sigma. 
%                 % s = C[\sigma(l)]_p \land
%                 % t = C[\sigma(r)]_p
%               \right \}
...
%     Let $W$, $\mathfrak{W}$ and $\mathfrak{I}$ be the class of ACTRS witnesses, the witness mechanism and the interpretation function, respectively, defined in \autoref{def:trs:ac}.
...
\chapter{Algebraic Rewriting Systems} 
    ...
    \begin{definition}[Rewriting rule, Match~\cite{ehrig1997algebraic}]
      ...
      \begin{definition}[Witness, Rewriting step \cite{endrullis2024generalized}]
        ...
        \begin{example}
            ...
            The DPO diagram below defines a rewriting step using the rule from \autoref{ex:grsaa}.
            \begin{center} 
                \resizebox{0.7\textwidth}{!}{
                \begin{tikzpicture}
    ...
    \end{center}
      The rewriting step in \autoref{ex:rewriting_step_grs_aa} is not possible with this rule in DPO rewriting with negative application conditions, because there exists a monomorphism \( q: N \rightarrow G \) such that \( n \star q = m \) as shown in the diagram below.
      \begin{center} 
        \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
          \graphbox{$N$}{-40mm}{0mm}{34mm}{20mm}{2mm}{-5mm}{
    ...
    \begin{definition}[\pbpop rewrite step~\cite{overbeek2023pbpo_JLAMP}]
      ...
      Let $\rho$ be a \pbpop rewriting rule as illustrated in \autoref{def:pbpop:rule}. 
    ...
    \begin{definition}[Rewriting framework]
      A \pbpop \textbf{rewriting framework} $\mathfrak{F}$ is a mapping associating to each \pbpop rule $\rho$ as depicted in \autoref{def:pbpop:rule} 
       the class of \pbpop witnesses of form as illustrated in \autoref{def:pbpop:rule}.
    \end{definition}
