...
\maketitle
\note{  
    Madames et Messieurs, je vous remercie d'etre présents.
...
Modeling of distributed systems
 ...
 \hspace{2cm}graph transformation based on \begin{tikzpicture}[baseline=-1ex]
...
\noindent Loop:\resizebox{0.85\textwidth}{!}{
    ...
    \note{
        Voici un exemple d'une règle de transformation de graphes qui échange les couleurs de deux arêtes orientées. En appliquant cette règle aux graphes en bas, on peut obtenir un cycle infini de transformations.

        De nombreux travaux existent sur la terminaison des systèmes de réécriture de termes. 
        
        Cependant, souvent ces méthodes ne s'appliquent pas directement aux systèmes de transformation de graphes,
        en raison de la présence possible de circuits dans les graphes. 
    }
\end{frame}
...
en interprétant les graphes par le nombre d'arêtes, par le nombre de nœuds, et par le nombre d'arêtes étiquetées par a.
...
\begin{frame}{Graph Morphisms: Structure-Preserving Functions}
    ...
    \note{
Un morphisme de graphes est une fonction qui envoie les nœuds et les arêtes d'un graphe vers ceux d'un autre en préservant la structure des graphes.

Nous utilisons des couleurs pour montrer la correspondance 
et des numéros pour préciser la correspondance des nœuds, si nécessaire.
    45s
    }
\end{frame}
...
\begin{frame}{Commutative Diagram}
    ...
    \end{beamercolorbox}
    \note{
    Considerons le diagramme de morphismes en haut.

    On dit qu'il commute parce que la composition des morphismes le long du chemin b puis a est égale à la composition des morphismes le long du chemin d puis c.
...
\begin{frame}{Pushouts: Gluing Graphs Along an Interface}
    ...
    \end{center}
    \note{
        quand $\alpha$ et $\beta$ sont injectives, 
        on peut illustré le rectangle de pushout comme le diagramme en haut a droite.
       
        On
          voit les graphes B et C comme deux extensions de l'interface A.
...
\note{
    ...
    Pour appliquer cette regle a un graphe G, 
    on cherche un morphisme injectif de L vers G, appelé le match.
    ...
    Enfin, on construit un deuxième diagram de pushout.
...
Cette méthode a un paramètre (T), appelé le graphe de type pondéré.
...
afin de donner une intuition aux deux dernières questions, 
...
\begin{frame}{Type Graph with Weights on Edges}
    ...
    \note{
        Nous fixons un graphe avec des poids sur les arêtes, c'est notre graphe de type pondéré T.
...
\begin{frame}{Usability Improvement}
    ...
    \note{ 
        ...
        Nous proposons d'utiliser des nombres réels positifs comme poids.
        
        Une contrainte supplémentaire est toutefois nécessaire: il doit exister un delta strictement positif tel que chaque étape de réécriture diminue le poids d'au moins delta.
...
%   \end{itemize} 
  ...
  \begin{itemize}
    \item impossible to guess $k$ and \crossout[red]{maximum weight $n$}
    \item complexity: \crossout[red]{$2^{k^2m} \cdot n^{k^2m}$ possible assignments of weights} \textcolor{blue}{$2^{k^2m}$ linear programs with $k^2m$ variables which have polynomial-time average-case complexity.}
  \end{itemize} 
...
\begin{frame}{Implementation$\leadsto$Experiments}
 ...
 \resizebox{0.5\textwidth}{!}{\textbf{Usability Improved~\textcolor{PineGreen}{$\checkmark$}}}
    ...
    \note{ 
        Nous avons implémenté la méthode dans un outil, 
        et mené des expériences pour évaluer l'amélioration de la performance lors de l'utilisation de poids dans les réels positifs par rapport à l'utilisation de poids dans les entiers naturels.
        
        Dans le tableau, 
        une case en vert indique une amélioration de performance 

        une case en rouge indique une dégradation 
        ...
        On remarque que pour les configurations 1 et 2, non seulement l'utilisateur n'a plus besoin de deviner le poids maximum, mais en plus 
        la performance est améliorée dans la majorité des cas.

        L'utilisabilité de la methode est donc améliorée.
...
\textcolor{red}{All existing automated methods fail.}
 ...
 \note{
    On remarque cependant que la méthode par graphe de type pondéré a une limitation importante: 
    elle ne peut pas prouver la terminaison d'une règle, comme celle illustrée,
    du au fait qu'il existe une surjection de R vers L, 
...
\begin{frame}{Capability Improvement: Morphism Counting}
    ...
    \note{
        Nous proposons une nouvelle methode de termination basée sur le comptage de morphismes.

        Cette methode utilise un graphe X comme parametre.

        L'interpretation d'un graphe G est le nombre de morphismes de X vers G.

        Pour présenter cette méthode, quelques concepts préliminaires et une notion de réécriture de graphe plus restrictive sont nécessaires.
    }
\end{frame}
...
\begin{frame}{Inclusions}
 ...
 Subgraph
 \note{
 Avant de presenter la nouvelle méthode, un concept preliminaire est nécessaire.
 
On appelle une inclusion un morphisme dont le domaine est un sous-graphe de son codomaine 
et chaque noeud et arête est envoyé à lui-même.
 }
...
Dangling edges
 \note{
    On appelle un prégraphe un graphe dans lequel certaines arêtes sont dans le vide. Un exemple est illustré en bas. 
    Ce pregraphe est obtenu en supprimant un nœud du graphe en haut.
 }
\end{frame}
...
La décomposition pour une règle abstraite est illustrée en bas.
   }
\end{frame}
...
Dans un diagramme de double-pushout définissant une étape de réécriture, 
chaque graphe peut être décomposé en une réunion de prégraphes, 
comme le montre l'exemple. 
Le cas abstrait est illustré en bas.

On peut colorier ces prégraphes avec des couleurs différentes. 
Cette coloration fournit une classification intuitive des morphismes vers des graphes dans une étape de réécriture, 
...
\begin{frame}{$X$-occurrences by Image Node Colors}
 ...
 \begin{center}
   ...
   \end{itemize}
    ...
    Pour faciliter la discussion, on introduit la notion de X-occurrence.
    On appelle une X-occurrence un morphisme injectif depuis X.
...
\note{ 
  Considerons les $X$-occurrences dans cet exemple.
...
\end{itemize} 
 ...
 \note{
  On peut maintenant énoncer une nouvelle condition suffisante de terminaison.
 
  une règle de réécriture termine si elle contient strictement plus d'occurrences orange que d'occurrences rouges, 
...
\begin{frame}{Sufficient Condition for the Second Condition~[Qiu]}
    ...
    \note{
        ...
        Si tous les sous-graphes du graphe en haut à droite qui peuvent former une occurrence bleu-et-rouge dans une étape de réécriture peuvent être envoyés vers des sous-graphes distincts du graphe en haut à gauche en préservant les éléments de l'interface K,
        alors il y a plus d'occurrences bleu-et-orange que d'occurrences bleu-et-rouge.
    }
\end{frame} 
...
\begin{frame}{Relative Termination: Intuition}
...
\begin{center}
            ...
            %  \resizebox{0.9\textwidth}{!}{
             \begin{tikzpicture}[baseline=-7ex]
                    \graphbox{\(\)}{0mm}{-3mm}{34mm}{15mm}{2mm}{2mm}{
                        ...
                        \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {$1$};
                        \node[draw,circle] (l2) at ($(l1)+(2,0)$) {$2$};
                        \draw[red,->,very thick] (l1) -- (l2) node[midway,above] {};
                    } 
            
                    \graphbox{\(\)}{40mm}{-3mm}{34mm}{15mm}{2mm}{2mm}{
                        \coordinate (o) at (0mm,-11mm); 
                        \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {$1$};
                        \node[draw,circle] (l2) at ($(l1)+(2,0)$) {$2$};
                    }  
            
                    \graphbox{\(\)}{80mm}{-3mm}{35mm}{15mm}{5mm}{2mm}{
                        \coordinate (o) at (-5mm,-11mm); 
                        \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {$1$};
        ...
        $\beta$={
            %  \resizebox{0.9\textwidth}{!}{
             \begin{tikzpicture}[baseline=-7ex]
                    ...
                    \graphbox{\(\)}{80mm}{-3mm}{35mm}{15mm}{5mm}{2mm}{
                        \coordinate (o) at (0mm,-11mm); 
                        \node[red,fill=red,draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {};
                    }    
                    \node () at (37mm,-10mm) {\( \leftarrow \)}; % K -> L
                    \node () at (77mm,-10mm) {\( \rightarrow \)}; % K -> R
                \end{tikzpicture}
    ...
    \note{
        Avant de présenter comment notre outil fonctionne, je vais expliquer brièvement le concept de terminaison relative.
    }
\end{frame}
...
\note{
    Voici le flux de processus de LyonParallel.
    ...
    Chaque méthode tente d'éliminer des règles.

    Si au moins une règle a été éliminée, le programme vérifie si le système est vide.
    Si c'est le cas, le systeme de recritrure de graphe se termine; sinon, l'utilisateur peut choisir une méthode pour continuer l'élimination.
...
%     \begin{center}
%         \includegraphics[width=0.9\textwidth]{plump18_copy_variant.png}
%     \end{center}
% \end{frame}
...
\begin{frame}{Conclusion and Future Work}
  ...
  \begin{itemize}
    ...
    \item New termination criterion,
    \item Extension of the new termination criterion (not presented),
    \item Implemention of these contributions.
  \end{itemize}
    \vspace{4mm}
  \textbf{Future work}
  \begin{itemize}
    \item Short term: comparison with the Subgraph-Counting method for PBPO+.
  ...
  \note{
    ...
    À long terme, nous souhaitons étendre les méthodes par graphe de type pondéré et countage de morphisms à d'autres formalismes de réécriture de graphes, tels que PBPO+.
