\documentclass[draft=true]{beamer}
  ...
  \begin{itemize}
    ...
    \item Aligns with the notion of program termination: 
         \begin{center}
          ...
          \end{center}
    \item Undecidable in general~\cite[Plump]{plump1995ontermination}
    %    \begin{itemize}
    %     \item Automated techniques for specific subclasses
    %    \end{itemize}
    \item Automated techniques   
        \begin{itemize}
            \item rely on user-privided parameters
            \item incomplete  
        \end{itemize}
  \end{itemize}
    \note{
...
\begin{frame}{Termination by interpretations~\cite[Zantema, Nipkow]{zantema2014termination,nipkow1998term}}
...
\begin{frame}{Graph Morphisms: Structure-Preserving Functions~\cite[Barr et al.]{barr1990category}}
             \begin{center}
          \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}
...
\begin{frame}{Commutative Diagram~\cite[Barr et al.]{barr1990category}}
    \begin{beamercolorbox}[rounded=true,shadow=true,wd=\textwidth]{block body}
        \begin{center}
            \resizebox{0.3\textwidth}{!}{
                \begin{tikzpicture}
...
\end{frame}
\begin{frame}{Pushouts: Gluing Graphs Along an Interface~\cite[Pierce]{pierce1991basic}}
    \begin{center}
    \begin{tikzpicture}[scale=0.8]
            \node (i) at (0,0) {A};
            \node (r) at (0,-2) {B};
...
\section{Formal Definition of Graph Rewriting with Double-Pushout}

\begin{frame}{Graph Rewriting with Double-Pushout (DPO)~\cite[Ehrig et al.]{ehrig1973graph}}
    \begin{description}
...
\begin{frame}{Weighted Type Graph Method~\cite[Endrullis et al.]{endrullis2024generalized_icgt}}
...
\begin{frame}{Termination Criterion~\cite[Bruggink et al.]{bruggink2015proving}}
    %   \begin{beamercolorbox}[rounded=true,shadow=true,wd=\textwidth]{block body}
    \begin{center}
    \begin{tikzpicture}
            \node (k) at (0,0) {$K$};
...
\begin{frame}{Searching for Weighted Type Graphs over $\mathbb{N}$~\cite[Bruggink et al.]{bruggink2015proving}}
...
existential Presburger arithmetic theory} with:
  ...
  Challenge:
  \begin{itemize}
    \item Usability: difficult to guess $k$ and $n$
    \item Search space: $2^{k^2m} \cdot n^{k^2m}$ possible assignments of weights
  \end{itemize} 
...
\begin{frame}{Usability Improvement}
    Using positive real numbers as weights~\cite[Lucas]{lucas2006relative}
...
\begin{frame}{A Limitation of the Weighted Type Graph Method}
    ...
    \end{center}
Type graph fails: existence of surjections from $R$ to $L$~[\citeauthor{endrullis2024generalized_icgt}].
...
\begin{frame}{A New Sufficient Condition for Termination~\cite[Qiu]{qiu2025termination_icgt}}
    \begin{center}
    \resizebox{0.5\textwidth}{!}{
            \begin{tikzpicture} 
                \coordinate (k) at (0, 0);
   ...
   \end{center}
    terminates if there are strictly more \textcolor{orange}{orange} X-occurrences than \textcolor{red}{red} X-occurrences,
    \begin{itemize}
        % \item  there are strictly more \textcolor{orange}{orange} X-occurrences than \textcolor{red}{red} X-occurrences,
        \item every rewriting step:
    \begin{center}
    \resizebox{0.5\textwidth}{!}{
            \begin{tikzpicture} 
                \coordinate (k) at (0, 0);
...
\begin{frame}{Sufficient Condition for the Second Condition~\cite[Qiu]{qiu2025termination_icgt}}
    \begin{center}
        \begin{tikzpicture} 
            \coordinate (k) at (0, 0);
            \draw[fill=white] ($(k)+(0,0)$) rectangle ($(k)+(0.5,0.5)$);
...
More power if search in parallel \textcolor{PineGreen}{\ding{51}}
