...
\begin{frame}{Motivation \& Goal}
    ...
    \begin{description}
        \item[Distributed/concurrent systems are everywhere.] 
    ...
    \begin{center}
      \resizebox{\textwidth}{!}{
       \begin{tikzpicture}[baseline=-10mm]
          ...
          \graphbox{$R$}{40mm}{0mm}{45mm}{15mm}{2mm}{-5mm}{
...
\begin{frame}{Pushouts: gluing graphs along a common part}
    ...
    \begin{itemize}
        ...
        \item<2-> $\square$\text{ABDC} is commutative,
        ...
        \item<3->{universality:}  \textcolor{PineGreen}{\( \forall (\gamma,\gamma').~\text{$\square$ABEC is commutative} \Longrightarrow \exists~!~\delta.~ \text{$\triangle$BDE \& $\triangle$CDE are commutative} \)}.
    \end{itemize} 
\end{definition} 
...
%     \item How to deal with the unknown context graph in a rewriting step?
%     \end{itemize}
% \end{frame}
...
\begin{frame}{Solution}
    ...
    \vspace{0.5cm}
    Constraint: there is \textcolor{red}{$\delta > 0$}, every rewriting step strictly decreases the weight by at least \textcolor{red}{$\delta$}.
...
\begin{frame}{A sufficient condition for termination}
 ...
 \textcolor{red}{Challenge}: the \textcolor{red}{second} condition.
\end{frame} 
...
\begin{frame}{Analysis of Implicit Occurrences}
    ...
    \begin{lemma}[More $X$-occurrences before rewriting]
       ...
       \begin{itemize}
        \item subgraphs of $R$ that can form an implicit $X$-occurrence in some rewriting step can be mapped to distinct  subgraphs in $L$ while 
...
\begin{frame}{A Limitation of the Weighted Type Graph Method}
    ...
    Termination proved :
    \begin{itemize}
        \item Number of explicit occurrences from $\tikz[baseline=-0.5ex]{ 
                \node (x) at (0,0) {$\bullet$}; 
                \node (y) at (1,0) {$\bullet$};
                \node (z) at (2,0) {$\bullet$};
                \draw[->] (x) -- (y) node[midway, above] {$s$};
        ...
        \item More implicit occurrences before rewriting
    \end{itemize}
...
\begin{frame}{Comparison with the weighted type graph method}
    \begin{description}
        \item[Morphism counting method has simpler conditions.]
    \end{description}
\end{frame}
