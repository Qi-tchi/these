...
\begin{frame}{DPO graph rewriting with injective rules}
 \begin{itemize}
    \item Rules $\varphi \mathop{=} (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R)$ consist of two inclusions $l$ and $r$.  
    \item A rule $\varphi' \mathop{=} (L' \overset{l'}{\leftarrowtail} K' \overset{r'}{\rightarrowtail} R')$ is equivalent to $\varphi$ if 
            \begin{center}
                ...
                \end{center}
    \item 
 Rewriting steps $G \mathop{\Rightarrow}_\varphi H$ are double-pushouts
%   with injective match $m :L' \rightarrowtail G$:
    ...
    %   \item Difference with standard definition: an equivalent rule $\varphi'$ with $\opn{lhs}(\varphi') \mathop{\subseteq} G$ is applied instead of $\varphi$.  
 \end{itemize} 
\end{frame} 
...
%     \begin{itemize}
%         \item \( \textcolor{red}{V} \mathop{\subseteq} \mathbb{N} \) : finite set of \textcolor{red}{nodes}

%         \item \( \textcolor{red}{E} \mathop{\subseteq} \mathbb{N} \) : finite set of \textcolor{red}{edges}

%         \item \( \textcolor{red}{\opn{src}} : E \mathop{\to} \mathbb{N} \) : \textcolor{red}{source} function

%         \item\( \textcolor{red}{\opn{dst}}: E \mathop{\to} \mathbb{N} \) : \textcolor{red}{target} function
...
\begin{frame}{Pre-graph operations}
    \textcolor{red}{Union} $C \mathop{\cup} R$ of two pre-graphs $C \mathop{\subseteq} G$ and $R \mathop{\subseteq} G$ 
    % \vspace{1mm}
\begin{center}
    \resizebox{\textwidth}{!}{
        \begin{tikzpicture}  
            ...
            \graphbox{\textcolor{red}{\(C \mathop{\cup} R \)}}{40mm}{-22mm}{45mm}{25mm}{2mm}{-3mm}{
    ...
    %     }         
    % \end{center}
\textcolor{red}{Relative complement} of $R$ in $H$ where $R \mathop{\subseteq} H$
, denoted $H \mathop{\setminus} R$: 
    % \vspace{1mm} 
  ...
  \resizebox{\textwidth}{!}{
        \begin{tikzpicture}  
            ...
            \graphbox{\textcolor{red}{\( H \mathop{\setminus} R  \)}}{40mm}{-22mm}{34mm}{25mm}{2mm}{-3mm}{
                \coordinate (o) at (0mm,-6mm); 
...
% \begin{frame}{Graph morphism $h : G \mathop{\rightarrow} H$}
...
%     Suppose $L \mathop{\cap} C \mathop{=} K$
%     % \begin{itemize}
%     %     \item $\alpha$, $\beta$ inclusions
%     %     \item $L \mathop{\cap} C \mathop{=} K$
%     % \end{itemize}

%     \textcolor{red}{Pushout} : \( L \overset{\beta'}{\rightarrowtail} L \mathop{\cup} C \overset{\alpha'}{\leftarrowtail} R \) with $\alpha'$, $\beta'$ inclusions\\

%     Example:

%     % Pushout object : $L \mathop{\cup} C$
...
%     \textcolor{red}{Pushout complement} : \( K \overset{\beta}{\rightarrowtail} (G\mathop{\setminus} L)\mathop{\cup} K \overset{\alpha'}{\leftarrowtail} G \) with $\alpha$, $\beta$ inclusions

%     Example: 

%     % Pushout complement object : $(G\mathop{\setminus} L)\mathop{\cup} K$: 
...
%             \graphbox{\( C \mathop{=} (G \mathop{\setminus} L) \mathop{\cup} K  \)}{40mm}{-20mm}{34mm}{27mm}{2mm}{-3mm}{
%                 \coordinate (o) at (0mm,-8mm); 
%                 \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {1};
%                 \node[draw,circle] (l2) at ($(l1)+(2,0)$) {2};
%                 \node[draw,circle,blue] (l4) at ($(l2)+(0,-1)$) {6};
...
%     $\alpha \mathop{=} L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R$ : rule

%     $\beta \mathop{=}  L' \overset{l'}{\leftarrowtail} K' \overset{r'}{\rightarrowtail} R'$ : rule
    
%     \textcolor{red}{Equivalent} if 
%     \begin{itemize}
%         \item there \textcolor{red}{exist isomorphisms $f,g,h$}
%         \item $l \circ g \mathop{=} f \circ l'$
%         \item $r \circ g \mathop{=} h \circ r'$
%     \end{itemize}
...
%                 \only<1->{
%                     \graphbox{\( C \mathop{=} \textcolor{red}{(G \mathop{\setminus} L) \mathop{\cup} K } \)}{40mm}{-22mm}{34mm}{25mm}{2mm}{-3mm}{
%                         \coordinate (o) at (0mm,-8mm); 
%                         \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {1};
%                         \node[draw,circle] (l2) at ($(l1)+(2,0)$) {2};
%                         \node[draw,circle] (l4) at ($(l2)+(0,-1)$) {6};
...
%                 \only<1->{
%                     \graphbox{\( H \mathop{=} \textcolor{red}{C \mathop{\cup} R} \)}{80mm}{-22mm}{45mm}{25mm}{2mm}{-3mm}{
%                         \coordinate (o) at (-5mm,-8mm); 
%                         \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {1};
%                         \node[draw,circle] (l2) at ($(l1)+(3,0)$) {2};
%                         \node[draw,circle] (l3) at ($(l1)+(1,0)$) {4};
...
%     % $L \mathop{=} L' \mathop{\cup} K$, $R \mathop{=} R' \mathop{\cup} K$, $C \mathop{=} C' \mathop{\cup} K$

%     % $G \mathop{=} L' \mathop{\cup} K \mathop{\cup} C'$, $H \mathop{=} R' \mathop{\cup} K \mathop{\cup} C'$
...
\begin{frame}{Running example}
    % Rule : a pair of inclusions $L \leftarrowtail K \rightarrowtail R$ with $L \mathop{\cap} R \mathop{=} K$
...
%     \begin{itemize}
%         \item \textcolor{red}{explicitly deleted} if $x \mathop{\subseteq} L' \mathop{\cup} K$ 
%         \item \textcolor{blue}{explicitly created} if $x \mathop{\subseteq} R' \mathop{\cup} K$
%         \item \hspace{1.57cm} shared if $x \mathop{\subseteq} C' \mathop{\cup} K$
%         \item \textcolor{red}{implicitly deleted} if $x \not \mathop{\subseteq} L' \mathop{\cup} K$ and $x \not \mathop{\subseteq} C' \mathop{\cup} K$
%         \item \textcolor{blue}{implicitly created} if $x \not \mathop{\subseteq} R' \mathop{\cup} K$ and $x \not \mathop{\subseteq} C' \mathop{\cup} K$
%     \end{itemize}
...
%     An $X$-occurrence is 
%     \begin{itemize} 
%         \item \textcolor{red}{explicit} if it is included in $L' \mathop{\cup} K$ or $x \mathop{\subseteq} R' \mathop{\cup} K$ 
...
\begin{frame}{Implicit, Explicit and Shared Occurrences}
 ...
 \begin{center}
   ...
   \end{center}
    ...
    \parbox{0.45\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{shared} if $y \mathop{\subseteq} C$}
    \parbox{0.45\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{explicit} if $y \mathop{\subseteq} L$}

    \parbox{\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{implicit} if $y \mathop{\cap} C' \mathop{\neq} \emptyset$ and \textcolor{blue}{$y \mathop{\cap} L' \mathop{\neq}  \emptyset$}}

    % \begin{itemize} 
    %     % \item \textcolor{red}{shared} if $y \mathop{\subseteq} C$ 
    %     % \item \textcolor{red}{explicit} if $y \mathop{\subseteq} L$  
    %     \item \textcolor{red}{implicit} if $y \mathop{\cap} C' \mathop{\neq} \emptyset$ and \textcolor{blue}{$y \mathop{\cap} L' \mathop{\neq}  \emptyset$}
    %     % if $y \not \mathop{\subseteq} L' \mathop{\cup} K$ and $y \not \mathop{\subseteq} C' \mathop{\cup} K$
    % \end{itemize}
    
    $X$-occurrence $y$ in $H$ is 

   \parbox{0.45\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{shared} if $y \mathop{\subseteq} C$}
   \parbox{0.45\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{explicit} if $y \mathop{\subseteq} R$ }

    \parbox{\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{implicit} if $y \mathop{\cap} C' \mathop{\neq} \emptyset$ and \textcolor{blue}{$y \mathop{\cap} R' \mathop{\neq} \emptyset$}}
    % \begin{itemize} 
    %     % \item \textcolor{red}{explicit} if $y \mathop{\subseteq} K \mathop{\cup} L'$ or $y \mathop{\subseteq} K \mathop{\cup} R'$ 
    %     % \item \textcolor{red}{explicit} if $y \mathop{\subseteq} L$ or $y \mathop{\subseteq} R$ 
    %     % \item \textcolor{red}{shared} if $y \mathop{\subseteq} C$
    %     % \item \textcolor{red}{implicit} otherwise (i.e. $y \mathop{\cap} C' \mathop{\neq} \emptyset$, and $y \mathop{\cap} L' \mathop{\neq}  \emptyset$ or $y \mathop{\cap} R' \mathop{\neq} \emptyset$)
    %     \item \textcolor{red}{shared} if $y \mathop{\subseteq} C$
    %     \item \textcolor{red}{explicit} if $y \mathop{\subseteq} R$ 
    %     \item \textcolor{red}{implicit} if $y \mathop{\cap} C' \mathop{\neq} \emptyset$ and \textcolor{blue}{$y \mathop{\cap} R' \mathop{\neq} \emptyset$}
    %     % if $y \not \mathop{\subseteq} L' \mathop{\cup} K$ and $y \not \mathop{\subseteq} C' \mathop{\cup} K$
    % \end{itemize}
\end{frame}
   ...
   \begin{center}
    \resizebox{0.9\textwidth}{!}{
      \begin{tikzpicture}
              ...
              \graphbox{\( H \mathop{=} R' \mathop{\cup} K \mathop{\cap} C' \)}{80mm}{-22mm}{45mm}{30mm}{2mm}{-10mm}{
                  \coordinate (o) at (-5mm,-3mm); 
                  \node[draw,circle] (l1) at ($(o)+(-10mm,0mm)$) {1};
                  \node[draw,circle] (l2) at ($(l1)+(3,0)$) {2};
                  \node[draw,circle,red] (l3) at ($(l1)+(1,0)$) {4};
...
\begin{frame}{Termination of a graph rewriting rule $\varphi$}
    \begin{itemize}
        ...
        \item $\varphi$ terminates if for all $G \mathop{\Rightarrow}_\varphi H$, the number of $X$-occurrences strictly decreases.
            ...
            % % \mathop{=} &\mathop{\mid}\text{explicit $X$-occurrences in $G$}\mathop{\mid} - \mathop{\mid}\text{explicit $X$-occurrences in $H$}\mathop{\mid}  +\\
            % %         &\mathop{\mid}\text{implicit $X$-occurrences in $G$}\mathop{\mid} - \mathop{\mid}\text{implicit $X$-occurrences in $H$}\mathop{\mid} \\
            % \mathop{>} & 0
            % \end{flalign*}
            % because $\text{shared $X$-occurrences in $G$} \mathop{=} \text{shared $X$-occurrences in $H$}$.
        ...
        \item $\varphi$ terminates if for all $G \mathop{\Rightarrow} H$,
        \begin{flalign*}
...
\begin{frame}{Termination of a graph rewriting rule}
   ...
   Therefore, the key challenge is to establish 
    $$\mathop{\mid}\text{implicit $X$-occurrences in $G$}\mathop{\mid} \mathop{\geq} \mathop{\mid}\text{implicit $X$-occurrences in $H$}\mathop{\mid}$$ 
    for all rewriting steps $G \mathop{\Rightarrow}_\varphi H$.
\end{frame}
...
%     $\mathop{\mid}\text{$X$-occurrences in $L$}\mathop{\mid} \mathop{=} 1 \mathop{>} 0 \mathop{=} \mathop{\mid}\text{$X$-occurrences in $R$}\mathop{\mid}$

%     \textcolor{red}{Terminating} if for all rewriting step $G \mathop{\Rightarrow} H$,
%         \begin{flalign*}
...
% %    Implicit $X$-occurrence $x$ in $G$ is of form $L'_x \mathop{\cup} K_x \mathop{\cup} C'_x$ 
     
%    Implicit $X$-occurrence $x$ in $H$ is of form $R_x \mathop{\cup} C'_x$ with $R_x \mathop{\subseteq} R$ and $C'_x \mathop{\subseteq} C'$
...
%     If  
%     \begin{itemize}
%         \item for every implicit $X$-occurrence $R_x \mathop{\cup} C'_x$, there is $h_x:R_x \rightarrowtail L$
%         \item $h_x$ preserves interface elements,
%     \end{itemize}
%     then $h_x(R_x) \mathop{\cup} C'$ is an implicit $X$-occurrence in $G$
% \end{frame}
...
\begin{frame}{Observation:For every $R_i \mathop{\in} D(R,X)$, there is a morphism \(h_i: R_i \rightarrowtail L \) which preserves the interface elements}
...
\begin{frame}{$X$-non-increasing rule}
 \begin{definition}
    ...
    \begin{enumerate}
        ...
        % \item For all $R'_i \mathop{\in} D(R,X)$, morphism $h_i$ does not maps any non-interface element to an interface element,
        % \item Different edges $e_i \mathop{\in} R_i$ and $e_j \mathop{\in} R_j$ have different images $h_{R_iL}(e_i) \mathop{\neq} h_{R_jL}(e_j)$,
        % \item If $X$ has isolated nodes, then then for all $R',R'' \mathop{\in} D(R,X)$ and nodes $x \mathop{\in} R', y \mathop{\in} R''$, if $x \mathop{\neq} y$, then $h_{R'L}(x) \mathop{\neq} h_{R''L}(y)$.
...
% \begin{frame}{Main results}
%     $\varphi \mathop{=} L \leftarrowtail K \rightarrowtail R$ : a rule
