...
\begin{frame}{Double-pushout (DPO) rewriting with injective rules and matches}
 \begin{itemize}
    ...
    \item Rules $\varphi \mathop{=} (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R)$ consist of two \alert{inclusions} $l$ and $r$.  
    \item Rule $\varphi' \mathop{=} (L' \overset{l'}{\leftarrowtail} K' \overset{r'}{\rightarrowtail} R')$ and $\varphi$ are \alert{equivalent} if there are \alert{isomorphisms $a,b,c$} such that:
            \begin{center}
...
\begin{frame}{Visualization of injective graph morphism}
        ...
        \begin{itemize}
            ...
            \item Some \alert{nodes} are \alert{numbered} for identification
            \item \alert{Morphism name} is placed \alert{in between the boxes}.
...
%    $y \mathop{\subseteq} R$ 
    ...
    \parbox{\textwidth}{\hspace{3mm}$\bullet$ \textcolor{red}{implicit} if 
    % $y \mathop{\cap} C' \mathop{\neq} \emptyset$ and $y \mathop{\cap} R' \mathop{\neq} \emptyset$
...
Consider the implicit occurrence of \tikz[baseline=-0.5ex]{ 
    ...
    The occurrence is $C' \mathop{\cup} R'$. $C'$ is shared by $G$ and $H$. $R'$ is not in $G$ but there $L'$ is in $G$ and $C' \mathop{\cup} L'$ is an implicit occurrence in $G$.
