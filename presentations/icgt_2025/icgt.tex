...
\begin{frame}{Main Results}
    \begin{lemma}[More $X$-occurrences before rewriting]
        If a rule $\varphi$ is $X$-non-increasing, then, for all $G \Rightarrow_\varphi H$, there are more implicit $X$-occurrences in $G$ than in $H$.
    \end{lemma}
         
    \begin{theorem}[Sufficient Termination Condition]
        If a rule $\varphi$ is $X$-non-increasing, then $\varphi$ is terminating if there are strictly more explicit $X$-occurrences in $L$ than in $R$.
    \end{theorem}
...
\begin{frame}{Relative work:todo}
    \begin{itemize}
        \item Type graph method Endrullis
        \item Subgraph method for PBPO+
    \end{itemize}
\end{frame}
