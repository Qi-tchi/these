...
\begin{frame}{DPO graph rewriting with injective rules}
 ...
 Rules $\varphi = (L \overset{l}{\leftarrowtail} K \overset{r}{\rightarrowtail} R)$ consist of two inclusions $l$ and $r$.   
 ...
 Rewriting steps $G \Rightarrow_\varphi H$ are double-pushouts:
      ...
      \end{center}
      where all arrows are inclusions and 
        \begin{center}
          \resizebox{0.40\textwidth}{!}{
              \begin{tikzpicture}
                    \node (I) at (0,0) {$K'$};
    ...
    Difference with standard definition: an equivalent rule $\varphi'$ with $\opn{lhs}(\varphi') \subseteq G$ is applied instead of $\varphi$.
\end{frame} 
...
\begin{frame}{Motivating example}
    ...
    Terminating by counting the occurrences of $\tikz[baseline=-0.5ex]{ 
                \node (x) at (0,0) {$\bullet$}; 
                \node (y) at (1,0) {$\bullet$};
                \node (z) at (2,0) {$\bullet$};
    ...
    A machine-checkable condition capturing this intuition is desired.
\end{frame}
...
\begin{frame}{Analysis of DPO diagram witnessing a rewriting step} 
...
\begin{frame}{$X$-occurrences in a graph $G$}
    ...
    \begin{flalign*}
     \mid\text{$X$-occurrences}\mid =
            &\mid\text{explicit $X$-occurrences}\mid + \\
            &\mid\text{shared $X$-occurrences}\mid + \\
            &\mid\text{implicit $X$-occurrences}\mid
    \end{flalign*}
\end{frame}
...
\begin{frame}{Termination of a graph rewriting rule $\varphi$}
    ...
    \begin{flalign*}
     &\mid\text{$X$-occurrences in $G$}\mid - \mid\text{$X$-occurrences in $H$}\mid \\
     = &\mid\text{explicit $X$-occurrences in $G$}\mid - \mid\text{explicit $X$-occurrences in $H$}\mid  +\\
            &\mid\text{implicit $X$-occurrences in $G$}\mid - \mid\text{implicit $X$-occurrences in $H$}\mid \\
     > & 0
    \end{flalign*}

    because $\text{shared $X$-occurrences in $G$} = \text{shared $X$-occurrences in $H$}$.

    Thus, $\varphi$ terminates if for every rewriting step $G \Rightarrow H$,
    \begin{flalign*}
        &\mid\text{explicit $X$-occurrences in $G$}\mid > \mid\text{explicit $X$-occurrences in $H$}\mid \\
        &\mid\text{implicit $X$-occurrences in $G$}\mid \geq \mid\text{implicit $X$-occurrences in $H$}\mid 
    \end{flalign*}
\end{frame}
...
\begin{frame}{Termination of a graph rewriting rule}
    ...
    is equivalent to 
        \begin{flalign*}
        \mid\text{$X$-occurrences in $L$}\mid > \mid\text{$X$-occurrences in $R$}\mid 
    \end{flalign*}
...
%     $\mid\text{$X$-occurrences in $L$}\mid = 1 > 0 = \mid\text{$X$-occurrences in $R$}\mid$

%     \textcolor{red}{Terminating} if for all rewriting step $G \Rightarrow H$,
%         \begin{flalign*}
%             \mid\text{implicit $X$-occurrences in G}\mid \geq 
%             \mid\text{implicit $X$-occurrences in H}\mid 
%         \end{flalign*}
% \end{frame}
...
\begin{frame}{$X$-non-increasing rule}
    ...
    Inclusions $h_1 : R_1 \rightarrowtail L$ and $h_2: R_2 \rightarrowtail L$ satisfy the conditions of the definition.
        % \begin{enumerate}
...
\begin{frame}{$X$-non-increasing rule}
    ...
    $$\mid\text{implicit $X$-occurrences in $G$}\mid \geq \mid\text{implicit $X$-occurrences in $H$}\mid$$ because
    \begin{description}
