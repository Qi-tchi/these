...
\begin{frame}{Weight of a morphism $h : L \to T$}
  \begin{itemize}
    \item $w_T(h)$ : the product of the weights of the edges of $\opn{Im}(h)$
    \item Example:
...
\begin{frame}{Not every graph $T$ can be a weighted type graph}

  There must be a morphism $c : L \to T$ such that

  For every rewriting step $G \Rightarrow H$, defined by
      ...
      \begin{center}
        ...
        \end{tikzpicture}
      \end{center}
  there is a morphism $\alpha : G \to T$ such that $c = \alpha \circ m$.

  To guarantee: $G$ has a weight and $w(G) \geq 0$, for all $G \Rightarrow H$
\end{frame}
...
%   \end{center}
  ...
  %           w_T(t_C - u) \right ) 
  %           \times
  %         \left (\sum_{\substack{t_L: L \rightarrow T\\ t_K = l \star t_L}}
  %         w_T(t_L) \right )
  %         \\
  %     w_T(H) 
  ...
  %         w_T(t_C - u) \right ) 
  %         \times 
  %         \left ( \sum_{\substack{t_R: R \rightarrow T\\ t_K = r \star t_R}}
  %             w_T(t_R) \right ) \\
  % \end{flalign*}
  % }
\end{frame}
...
\begin{frame}{Searching a weighted type graph \( T \) in practice}
