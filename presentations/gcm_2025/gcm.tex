...
\begin{frame}{Type Graph Method with Weighted Type Graphs over Well-founded Semirings}
  % for proving termination of DPO rewriting systems on edge-labeled directed graphs
    ...
    \begin{itemize}
      ...
      \item Assigns weights $w_T(G)$ with respect to $T$ to graphs $G$
...
\begin{frame}{Type Graph Search in Practice and its Complexity
  %  for proving termination of DPO rewriting systems on edge-labeled directed graphs
      ...
      \end{itemize}
      Porblem reduced to solving an integer program
                \begin{itemize}
...
\begin{center} 
  ...
  \end{center}
   ...
   \item Termination proved by the following weighted type graph $T$ over Natural Arithmetic Semiring:
    % \begin{itemize}
    %   \item finite directed graph
    %   \item edge-labeled
    %   \item weights are positive real numbers
...
$$  
the product of the weights (in $ T $) of the images of the edges of $ L $ under the morphism $ h $.
...
\begin{frame}{Not every graph can be a weighted type graph}
  ...
  For every rewriting step $G \Rightarrow_\mathcal{R} H$:
  ...
  To guarantee: $w_T(G)$ is defined and $w_T(G) \geq 0$, for all $G \Rightarrow H$
\end{frame}
...
\begin{frame}{Detail for $t_K^{11}$}
  \begin{center}
      ...
      \end{center}
We have $w_\mathcal{T}(h_{11}^1) + w_\mathcal{T}(h_{11}^2)
        =(1 * 1) + (1 * 1)
        =2$
      ...
      \end{center}
    we have $w_\mathcal{T}(h_{11}^3) = 1 * 1 * 1  = 1$.

    Therefore, for $t_K^{11}$, we have: $2 - 1 \geq 1$.
\end{frame}
...
\begin{frame}{Observation}
  \begin{itemize}
    \item weights can be taken from $\mathbb{R}^+$
    \item $w_T(G) \geq 0$ for all $G \Rightarrow H$ \\ by definition of the type graph method 
  \end{itemize}
  \begin{proposition}
  A set of rewriting rules $\mathcal{R}$ terminates, if 

   \begin{enumerate}
      \item there exists $\delta \in \mathbb{R}$, $\delta > 0$
      \item $w_T(G) - w_T(H) \geq \delta$ for every rewriting step $G \Rightarrow_\mathcal{R} H$
   \end{enumerate} 
  \end{proposition}
...
\begin{frame}{
  Termination of the running example with a weighted type graph over non-well-founded semiring  }
  ...
  \item 
  Termination proved by the weighted type graph over Real Arithmetic Semiring:
    \begin{center}
        \begin{tikzpicture}
            \graphbox{}{0mm}{0mm}{32mm}{28mm}{-10mm}{-14mm}{
                \node[draw,circle] (1) at (0,0) {};
