...
\begin{center} 
  ...
  \end{center}
   ...
   Weighted type graph $T$ over positive real numbers:
    % \begin{itemize}
    %   \item finite directed graph
    %   \item edge-labeled
    %   \item weights are positive real numbers
...
\begin{frame}{Weight $w_T(h)$ of Morphism $h : L \to T$}
  ...
  Weight function $w : \opn{Edges}(T) \to \mathbb{R}^+$ 
...
We have $w_\mathcal{T}(h_{11}^1) + w_\mathcal{T}(h_{11}^2)
        ...
        \begin{tikzpicture}
          \graphbox{\( R \)}{-55mm}{0mm}{45mm}{44mm}{1mm}{-22mm}{
            ...
            \graphbox{$T$}{0mm}{0mm}{40mm}{44mm}{-10mm}{-22mm}{
                ...
                \node[draw,circle] (2) at (2,0) {};
                \draw[->,red] (1) edge[loop above] node[midway, above] {$a^{1.0}$} (1) ;
                \draw[->,red] (1) edge[loop below] node[midway, below] {$b^{1.0}$} (1) ;
                \draw[->] (1) edge[bend left] node[midway, above] {$a^{1.0}$}  (2)  ;
                \draw[->] (2) edge[bend left] node[midway, below] {$a^{1.0}$} (1)   ;
            }
      ...
      \end{center}
    we have $w_\mathcal{T}(h_{11}^3) = 1.0 * 1.0 * 1.0  = 1.0$.
...
\begin{frame}{Searching Weighted Type Graphs over the natural vs. real semirings}
    with $k$ nodes and no parallel edges of the same label from $\Sigma$
